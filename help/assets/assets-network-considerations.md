...
これはクライアント起因の最も混雑の激しい渋滞地点です。ただし、このファイアウォールを担当するネットワーク操作グループとの間で、変更または許可されたリストに対する評価を行うことができます。
...
Dispatcher では、100 Mbps の接続が 2 つのエンティティ（外部と AEM インスタンス）で共有されています。アップロードとダウンロードを同時に実行するには、この数を 2 で割る必要があります。接続された外部ストレージは別の接続を使用します。
