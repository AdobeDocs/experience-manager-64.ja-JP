...
アセットテンプレートはほとんど InDesign ファイルですが、InDesign の習熟は素晴らしい成果物の作成の障害にはなりません。InDesign テンプレートのフィールドを、カタログを作成する際には必要となる商品フィールドとマッピングさせる必要はありません。テンプレートは、WYSIWYGモードでWebインターフェイス上で直接編集できます。 ただし、InDesignで編集の変更を処理するには、まずAEM Assetsを設定してInDesignサーバーと統合する必要があります。
...
1. 販促物を編集するために開くには、次のいずれかの操作をおこないます。
   ...
   * コラテラルを選択し、ツールバーから「Edit」アイコンをクリックまたはタップします。
...
1. ツール/アセットをク **[!UICONTROL リックまたはタップしま]**&#x200B;す。
1. オプションから、「**[!UICONTROL テンプレート]**」を選択します。
