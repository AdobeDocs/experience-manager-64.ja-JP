...
AEMをWCMとして使用している場合は、Dynamic Mediaアセットを直接ページに追加します。WCMにサードパーティを使用している場合は、アセットを[リンク](linking-urls-to-yourwebapplication.md)または[埋め込む](embed-code.md)。サードパーティのレスポンシブWebサイトの場合は、[レスポンシブサイトへの最適化された画像の配信](responsive-site.md)を参照してください。
...
1. 「**[!UICONTROL コンポーネント]**」見出しの下にあるドロップダウンリストで、「**[!UICONTROL Dynamic Media]**」を選択します。 Dynamic Media コンポーネントのリストがない場合は、使用する Dynamic Media コンポーネントを有効にしなければならない可能性があります。詳しくは、[Dynamic Media コンポーネントの有効化](#enabling-dynamic-media-components)を参照してください。
...
1. [必要に応じてコ](#dynamic-media-components) ンポーネントを編集し、チェックマークをクリックして変更を保存します。
...
コンポーネント内の&#x200B;**[!UICONTROL 編集]**&#x200B;アイコンをクリックし、次に&#x200B;**[!UICONTROL Dynamic Media設定]**&#x200B;をクリックして、次のDynamic Media設定を編集する必要があります。

![dm-settings-image-preset](assets/dm-settings-image-preset.png)

>[!NOTE]
>
>デフォルトでは、Dynamic Media 画像コンポーネントはアダプティブです。画像コンポーネントを固定サイズにする場合は、コンポーネントの「**[!UICONTROL 詳細]**」タブで、「**[!UICONTROL 幅]**」と「**[!UICONTROL 高さ]**」の設定に従ってサイズを設定します。
...
ア修飾子viewer修飾子は、name=valueの形式で&amp;を区切り文字として使用し、『Adobeビューアリファレンスガイド』で概要を説明しているようにビューアを変更できます。Posterimage=img.jpg&amp;caption=text.vtt,1 はビューア修飾子の一例です。
   ...
   * ビデオ[キャプションにキャプションファイルを関連付けます。](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-aem-assets-dmc/video/command-reference-url-video/r-html5-video-viewer-url-caption.html?lang=ja)
   * ビデオ[ナビゲーションにナビゲーションファイルを関連付けます。](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-aem-assets-dmc/video/command-reference-url-video/r-html5-video-viewer-url-navigation.html?lang=ja)
