---
title: 翻訳プロジェクトの作成
description: AEMで翻訳プロジェクトを作成する方法を説明します。
contentOwner: AG
feature: Translation
...
>[!NOTE]
>
>PDFやInDesignファイルなど複雑なアセットの翻訳ワークフローを起動しても、サブアセットやレンディション（存在する場合）は翻訳のために送信されません。
...
1. Assets UI に移動し、翻訳済みアセットごとにプロパティページを開いて、翻訳済みメタデータを表示します。

   ![chlimage_1-74](assets/chlimage_1-74.png)

   >[!NOTE]
   >
   >この機能は、アセットに対してもフォルダーに対しても使用できます。フォルダーではなくアセットを選択すると、言語ルートまでのフォルダーの階層全体がコピーされ、アセットの言語コピーが作成されます。
...
翻訳ワークフローを実行して、元のアセットを編集したバージョンで言語コピーを更新するとき、翻訳済みアセットをユーザーが承認するまで、既存の言語コピーが維持されます。[!DNL Experience Manager] Assets は、新しく翻訳されたアセットを一時的な場所に保存し、アセットを明示的に承認した後で既存の言語コピーを更新します。 ユーザーがアセットを承認しないと、言語コピーは変更されません。
