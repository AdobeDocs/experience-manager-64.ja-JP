...
description: アセットを編集用にチェックアウトし、変更が完了したら再度チェックインする方法について説明します。
contentOwner: AG
...
# アセット内のファイルのチェックインとチェックアウト {#check-in-and-check-out-files-in-assets}

Adobe Experience Manager (AEM) Assetsを使用すると、アセットを編集用にチェックアウトし、変更が完了した後で再びチェックインできます。 アセットをチェックアウトした後は、そのアセットの編集、注釈の追加、公開、移動または削除を行うことのみ可能です。 アセットをチェックアウトすると、アセットがロックされます。 他のユーザーは、アセットをAEM Assetsに再度チェックインするまで、アセットに対してこれらの操作を実行できません。 ただし、ロックされたアセットのメタデータは変更することができます。
