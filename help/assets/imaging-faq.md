...
* PNGリクエストを実行して可逆圧縮画像を取得します。
* 様々な画質設定でのJPEG要求。
* JPEGXRは、様々な品質設定で要求します。
* JPEG2000リクエストを様々な画質設定で作成できます。
* WEBPリクエストは、様々な品質設定で実行されます。
...
スマートイメージングは、既存の画像プリセットと連携し、サイズ、画質、シャープなど、実質上すべての画像設定を監視します。変更されるのは、要求に応じた画像形式と画質設定です。 形式変換の場合、画像プリセットの設定に基づいて完全な視覚的忠実度が維持されますが、ファイルサイズは小さくなります。
...
* ダイナミックメディアを使用している場合は、「ダイナミックメディア [イメージの設定](config-dynamic.md) 」の「有効期限 **** の値を24時間以上に設定」の指示に従います。
