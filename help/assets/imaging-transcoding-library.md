...
1. コマンドを使用してフォルダーをチェックし、必要なすべてのライブラリが抽出されているこ `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/`とを確認します。ここで、フォルダー名はバンドルIDを使用して構築されます。 例えば、コマンドは `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle588/data/binaries/` if bundle idがです `588`。
...
1. confファ `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` イルにパスを追加するには、コマンドを使 `cat SWitchEngineLibs.conf` 用します。
