...
1. コマンドを使用してフォルダーをチェックし、必要なすべてのライブラリが抽出されていることを確認します。ここで `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/`、フォルダー名はバンドルIDを使用して構築されます。 例えば、バンドルIDが `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle588/data/binaries/` である場合、コマンドはで `588`す。
...
1. confファイルへの追加パス( `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/``cat SWitchEngineLibs.conf` コマンドを使用)。
...
### DAM更新アセットの構成ワークフロー {#configure-dam-asset-update-workflow}
