...
**ラベル** ：ユーザーが図に表示するテキスト。 リッチテキストエディターで書式設定を変更できます。

**Target URL** ユーザーが画像をクリックした場合に訪問するURIを入力します。
...
**バージョン情報** ：ユーザーは「概要」情報を追加できます。

![chlimage_1-55](assets/chlimage_1-55.png)

**住所フィールド** ：ユーザーが住所情報を入力できます。 このコンポーネントを設定する場合、ダイアログの「エレメント名」に入力する必要があります。「エレメント名」はフォーム要素の名前です。リポジトリ内でデータが格納される場所を示します。

![chlimage_1-56](assets/chlimage_1-56.png)

**生年月日ユーザーは** 、生年月日情報を入力できます。
...
**名** ：ユーザーが姓を入力するためのフィールドです。

![chlimage_1-59](assets/chlimage_1-59.png)

**性別** ：ユーザーはドロップダウンリストから性別を選択できます。
...
**リードフォーム** ：このコンポーネントを追加して、ランディングページにリードフォームを追加します。 リードフォームには、「リードフォームの最初」フィールドと「リードフォームの終わり」フィールドが自動的に含まれます。これらのフィールドの間に、ここで説明しているリードフォームコンポーネントを追加できます。
...
**ワークフローの開始** ：リードフォームの送信後にトリガーされるワークフローを決定します。
...
* Email Service:自動応答電子メールの送信 — ExactTargetなどの電子メールサービスプロバイダーを使用している場合に使用します。
* Email Service:リストからユーザーを登録解除 — ExactTargetなどの電子メールサービスプロバイダーを使用している場合に使用します。
* ユーザーの購読を解除

**フォーム識別子** ：リードフォームを一意に識別するフォーム識別子。 1 つのページに複数のフォームがある場合、フォーム識別子を使用します。フォームごとに異なる識別子が使用されていることを確認します。

**ロードパス** ：リードフォームフィールドに定義済みの値をロードするために使用するノードプロパティへのパスです。
...
**クライアント検証** ：このフォームにクライアント検証が必要かどうかを示します（サーバー検証は常に行われます）。 これは、Forms Captchaコンポーネントと組み合わせて実行できます。

**検証リソース・タイプ** (Validation Resource Type):（個々のフィールドではなく）リード・フォーム全体を検証する場合に、フォーム検証リソース・タイプを定義します。
...
**アクションの設定** 「投稿オプション」での選択内容に応じて、「アクションの設定」が変更されます。 例えば、「リードを作成」を選択した場合、リードの追加先とするリストを設定できます。
...
1. ドラッグ＆ドロップをおこなうと、インポーターページの代わりにランディングページが表示されます。HTMLランディングページが正常にインポートされました。
...
変更をおこなった後でランディングページデザインパッケージを再インポートする場合は、クラシックユーザーインターフェイスのランディングページの上部にある「**消去**」をクリックして、ランディングページを「消去」したり、タッチ最適化ユーザーインターフェイスの設定メニューで「消去」をクリックしたりすることができます。この操作を行うと、読み込んだランディングページが削除され、空のインポーターページが作成されます。
