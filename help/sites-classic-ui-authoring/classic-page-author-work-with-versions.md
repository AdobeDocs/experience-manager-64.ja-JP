...
1. Click **Create Version**. 灰色のメッセージが表示されます。以下に例を示します。
...
1. **バージョンを復元…**現在のフォルダー内のドキュメントのバージョンを表示します。ページが削除されても、最後のバージョンが表示されます。
...
1. 現在のバージョンと選択したバージョンの違いは次のとおりです。
...
* つまり、Timewarp で選択した時点より前に作成またはアクティベートされたバージョンが表示されます。**
...
1. 日付と時間のセレクターを使用して目的の日付や時間を指定し、「**移動**」をクリックします。
   ...
   >[!NOTE]
   >
   >Timewarp は対象のページが以前に発行されている場合のみ完全に機能します。以前に公開されていない場合、現在のページがオーサー環境で表示されます。
...
1. タイムラインを選択して動かし（クリックしながらドラッグ）、ドキュメントのタイムライン内を移動します。
   ...
   * 線はすべて発行済みのバージョンを示します。

      ページがアクティベートされると、新しい線が始まります。ドキュメントが編集されるたびに、新しい色が表示されます。
...
Timewarpを使用すると、選択した時点でページを再現するための最善の方法が得られます。 ただし、AEMでコンテンツを継続的にオーサリングするのは複雑なので、これは常に可能とは限りません。 Timewarpを使用する際は、以下の制限事項を考慮する必要があります。

* **Timewarpは、公開されたページに基づいて機能します** - Timewarpは、以前にページを公開した場合にのみ完全に機能します。 以前に公開されていない場合、現在のページがオーサー環境で表示されます。
* **Timewarpはページバージョンを使用** — リポジトリから削除または削除されたページに移動すると、そのページの古いバージョンがリポジトリで引き続き使用可能な場合は、正しくレンダリングされます。
* **削除されたバージョンはTimewarpに影響** — バージョンがリポジトリから削除されると、Timewarpは正しいビューを表示できません。
* **Timewarpは読み取り専用** — 古いバージョンのページは編集できません。 古いバージョンは表示のみ可能です。古いバージョンを復元する場合は、復元を使用して手動でおこなう必要があります。
* **Timewarpはページのコンテンツに基づいてのみ行われます** - webサイトのレンダリング用の要素（コード、CSS、アセット/画像など）が変更された場合、これらの項目はリポジトリでバージョン管理されないので、表示は元の表示と異なります。

>[!CAUTION]
>
> Timewarpは、作成者がコンテンツを理解し、作成できるよう支援するツールとして設計されています。 監査ログや法的な目的のためのものではありません。