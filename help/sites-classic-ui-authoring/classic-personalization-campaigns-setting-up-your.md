...
* **優先度：**&#x200B;他のキャンペーンと比較した場合の、このキャンペーンの優先度です。複数のキャンペーンが同時にオンの場合、優先度が最も高いキャンペーンが訪問者エクスペリエンスを制御します。
...
1. 「サンプルデータ」の説明に従って、情報を入力します。読み込むことができるフィールドは、email、familyName、givenName、gender、aboutMe、city、country、phoneNumber、postalCode、region、streetAddress です。
   ...
   >
   >`email,givenName,familyName`  — 書き込み済みの場合 `givenname`例えば、システムが認識しない場合などです。
