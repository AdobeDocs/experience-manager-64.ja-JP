...
ページをカルーセルに含める方法を指定します。
   ...
   * **リストを選択**
      ...
      どの方法を選択しても、リストに含める各ページには、そのページに関連付けられた画像が既に存在する場合は、その画像がカルーセルに表示されます。 ページのページプロパティの下に特定のページの画像がない場合は、開始する前に画像をページに関連付ける必要があります。画像がない場合は、空白の（ほとんど空白の）ページがカルーセルに表示されます。 See [Editing Page Properties](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md).
      ...
      * **固定リストのオプション**
         ...
         * **ページ**
            ...
            ページのリストを選択します。 Use `+` to add more entries and the up/down buttons to adjust the order.
      * **検索のオプション**
...
>[!CAUTION]
>
>コンテンツフラグメント管理の全機能は、タッチ操作対応UIでのみ使用できます。
>
...
* **リスト**
   ...
   * 選択する項目によって、新しいパネルが表示されます。
      ...
      * **検索のオプション**
         ...
         * **開始場所** — 開始パスを手動で入力するか、セレクターを使用します。 検索クエリ
プレーンテキストの検索クエリを入力できます。
      * **詳細検索のオプション**

         * **「Querybuilder predicate notation** 」 - Querybuilder predicate notationを使用して検索クエリーを入力できます。例えば、「fulltext=Marketing」と入力すると、コンテンツに「Marketing」が含まれるすべてのページがカルーセルに表示されます。クエリ [式の詳細とその他の例については](/help/sites-developing/querybuilder-api.md) 、QueryBuilder APIを参照してください。
...
* ビデオ
   ...
   * **サイズ** — ビデオのネイティブサイズ(幅x高さ（ピクセル単位）は、「サイズ」の横のボックスに表示されます（上記を参照）。 ビデオのネイティブサイズを上書きする場合は、ここに幅と高さの寸法を手動で入力します。[ **OK** ]をクリックしてダイアログを閉じます。
...
* **制約**
   ...
   * **必須メッセージ** — このフィールドが必須であることをユーザーに通知するメッセージ。必須フィールドにも、とアスタリスクが付きます。
...
* **アドバンス**
   ...
   * **アクションタイプ**
      ...
      フォームにはアクションが必要です。アクションは、ユーザーから送信されたデータによってトリガーされ実行される処理を定義するものです（HTML の action= に似ています）。対応するアクション設定が必要 **な場合もありま**&#x200B;す。
