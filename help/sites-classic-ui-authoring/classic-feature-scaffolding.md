...
*次に、この基礎モードで作成するページの構造を定義する必要があります。 定義を行うには、基礎モードページで「デザインモード」に切り替えます。リンクが表示され、**ダイアログエディター**&#x200B;で基礎モードを編集できます。
...
&quot;これは、この段落のテキストコンテンツをレンダリングするために使用されるコンポーネントが *テキスト画像* コンポーネント。 他の非表示フィールドで指定されている `isRichText` ブール値を使用すると、`./jcr:content/par/text/text` に格納された実際のテキスト文字列を希望の方法でコンポーネントがレンダリングできます。
