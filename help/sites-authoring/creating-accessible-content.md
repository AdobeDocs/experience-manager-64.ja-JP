...
* 説明写真：人や物や場所の画像です。ページ内の写真の役割を考えてみましょう。該当するテキストは、*[object]*&#x200B;の写真と見なされますが、周囲のテキストに依存する場合があります。
...
* CAPTCHA:CAPTCHAは、コンピュータと人間をApart *に伝える完全に自動化されたパブリックチューリングテストです。これは、悪意のあるソフトウェアと人間を区別するためにWebページで使用されるセキュリティチェックですが、アクセシビリティの障害の原因となる可能性があります。画像とは、セキュリティテストに合格するために、ユーザーに表示される内容の説明を求める画像です。画像に代替テキストを指定することは明らかに不可能なので、代わりにグラフィック以外の代替ソリューションを検討する必要があります。*
...
* 背景画像：これらの指標は、HTMLではなく、カスケーディングスタイルシート(CSS)を使用して実現されます。これは、代替テキスト値を指定できないことを意味します。したがって、背景画像は重要なテキスト情報を提供しないでください。提供する場合、この情報はページのテキストにも提供する必要があります。
...
* *標準*&#x200B;の2次元、複数列、色付きのWebページレイアウトで、作成者が提示した情報にアクセスできない場合があります。
...
* **見出しを使用：**
   ...
   RTEのアクセシビリティ機能を有効にしている限り([AEMおよびアクセシビリティ](#AdobeExperienceManagerandAccessibility)を参照)、AEMオファー3レベルのページ見出しを使用できます。これらを使用して、コンテンツのセクションやサブセクションを識別できます。見出し1は見出しの最上位レベルで、見出し3は最下位です。システム管理者は、より多くの見出しレベルを使用できるようにシステムを設定できます。
...
* **テーブルを使用**：
   ...
   * データセルごとに 1 つの `<td>` 要素
   ...
   >[!NOTE]
   従来のUIでは、テーブルは&#x200B;**テーブル**&#x200B;コンポーネントで実現する必要があります。
...
* 一方、米国英語としてレンダリングされるページは、次の標準を採用する必要があります。`<html lang = “en-us”>`
