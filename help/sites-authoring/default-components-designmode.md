...
これらに加えて、他の様々なコンポーネントも使用できます。デザインモードを使用して、 [このようなコンポーネントを有効化/無効化](#enable-disable-components).有効にしてページ上に配置すると、デザインモードを使用して [コンポーネントデザインの側面の設定](#configuring-the-design-of-a-component) 属性パラメータを編集する。
...
>[!NOTE]
>
>デザインモードは、( `/etc`) をクリックします。
>
>AEM 6.4 以降では、設計を設定データとしてに格納することをお勧めします。 `/apps` 継続的なデプロイメントシナリオをサポートする に保存されたデザイン `/apps` は実行時には編集できず、このようなテンプレートに対して、管理者以外のユーザーはデザインモードを使用できません。
...
1. このダイアログを使用して、現在のページの編集時にコンポーネントブラウザーで利用できるコンポーネントを定義します。
   ...
   **許可されたコンポーネント**
   ...
   の **許可されたコンポーネント** 「 」タブでは、parsys で使用できるコンポーネントを定義できます。
...
1. ダイアログを開くには、**設定**&#x200B;アイコンを使用します。
   ...
   デザインダイアログで、使用可能なデザインパラメータに従ってコンポーネントを設定できます。
