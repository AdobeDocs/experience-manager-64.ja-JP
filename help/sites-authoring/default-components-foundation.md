...
>[!NOTE]
>
>この節では、基盤コンポーネントについて説明します。基盤コンポーネントは、AEM の多くのバージョンで利用でき、AEM の標準インストール環境でそのまますぐに使用できます。ただし、AEM 6.4では、多くの基盤コンポーネントが非推奨（廃止予定）となっています。
>
>Adobeでは、より最新で拡張可能な[コアコンポーネント](https://helpx.adobe.com/experience-manager/core-components/using/authoring.html)を活用することをお勧めします。 これらは[We.Retailサンプルコンテンツ](/help/sites-developing/we-retail.md)の一部であり、[個別にインストールし、管理者が開発用に](https://helpx.adobe.com/experience-manager/core-components/using/using.html)使用することもできます。

基盤コンポーネントは、標準の Web ページ用のコンテンツを作成するためのものです。AEM の標準のインストールでそのまま使用できるコンポーネントのサブセットを形成します。

コンポーネントブラウザーを通じてすぐに使用できるものもあれば、[デザインモード](/help/sites-authoring/default-components-designmode.md)（ページが静的テンプレートに基づいている場合）または[テンプレート](/help/sites-authoring/templates.md)の編集（ページが編集可能テンプレートに基づいている場合）を使用して使用できるものもあります。

基盤コンポーネントの使用はサポートされていますが、コアコンポーネントに置き換えられ、より拡張性と柔軟性が提供されます。
...
* [フォーム](#form)：フォームの作成に必要なすべてのコンポーネントを含みます。
...
>[!CAUTION]
>この基盤コンポーネントは廃止されました。Adobeでは、代わりに、[コアコンポーネントの画像コンポーネント](https://docs.adobe.com/content/help/ja/experience-manager-core-components/using/components/image.html)を利用することをお勧めします。
...
>[!CAUTION]
>
>&amp;ast;&amp;ast;アダプティブレンディションのAEMでは、アニメーション.gifファイルはサポートされていません。
...
* **カルーセル**
   ...
   * 遷移時間
      ...
      2つのスライド間の切り替えの時間（ミリ秒）。

   * コントロールのスタイル

      プルダウンメニューからは、様々なオプションを使用できます。例えば、前/次のボタン、右上スイッチなど。

* **リスト**

   ここでは、ページをカルーセルに含める方法を指定します。

   * **リストを選択**

      ページリストを作成する方法はいくつかあります。子ページ、固定リスト、検索、詳細検索（以下で説明）。

      どの方法を選択しても、リストに含めるページには、そのページに関連付けられた画像が既に存在するはずで、この画像がカルーセルに表示されます。 そのページのページプロパティの下に特定のページの画像がない場合、開始する前に画像をページに関連付ける必要があります。画像がない場合、カルーセルには空白（またはほとんど空白）のページが表示されます。 [ページのプロパティの編集](/help/sites-authoring/editing-page-properties.md)を参照してください。
...
手動で、またはセレクターを使用して、パスを指定します。現在のページを親として使用するには、空のままにします。
      * **固定リストのオプション**
         ...
         * ****
ページページのリストを選択します。用途 
`+` をクリックして、エントリを追加したり、上/下ボタンを使用して順序を調整したりします。
      * **検索のオプション**
         ...
         * **検索クエリー**
            ...
            プレーンテキスト検索クエリを入力できます。
      * **詳細検索のオプション**

         * **Querybuilder 述語の表記**

            検索クエリは、「 Querybuilder述語の表記」を使用して入力できます。 例えば、「fulltext=Marketing」と入力すると、コンテンツに「Marketing」が含まれるすべてのページがカルーセルに表示されます。
...
>
>AEMで使用されるHTML5の機能について詳しくは、「[サポートされるプラットフォーム](/help/release-notes/release-notes.md#supported-platforms)」の節を参照してください。
...
* ログイン
   ...
   * にリダイレクト
      ...
      ユーザーがログインした後に開くWebサイト上のページを指定できます。
...
* **段落参照**
   ...
   * **参照**
      ...
      参照するページと段落のパスを指定します（コンテンツを含めます）。
...
>[!CAUTION]
>この基盤コンポーネントは廃止されました。Adobeでは、代わりに、[ナビゲーション](https://docs.adobe.com/content/help/jp/experience-manager-core-components/using/components/navigation.html)、[言語ナビゲーション](https://docs.adobe.com/content/help/jp/experience-manager-core-components/using/components/language-navigation.html)および[パンくずコアコンポーネント](https://docs.adobe.com/content/help/jp/experience-manager-core-components/using/components/breadcrumb.html)を利用することをお勧めします。
...
>[!NOTE]
>
>**テーブル**&#x200B;コンポーネントは、[テキスト](/help/sites-authoring/rich-text-editor.md)コンポーネントと同様に、**[リッチテキストエディター](#text)**&#x200B;をベースとしています。
>
...
>[!NOTE]
>
>**テキスト**&#x200B;コンポーネントは、[テーブル](/help/sites-authoring/rich-text-editor.md)コンポーネントと同様に、**[リッチテキストエディター](#table)**&#x200B;をベースとしています。
>
>テーブルには&#x200B;**テーブル**&#x200B;コンポーネントを使用することを推奨しますが、**テキスト**&#x200B;コンポーネントでも作成できます。
...
次に、**タイトル**&#x200B;コンポーネントを表示した例を示します。サイト専用の CSS でデザインを決定しています。
...
HTML5要素で使用するビデオコンポーネント](/help/sites-administering/config-video.md)の設定も参照してください。[
...
* **制約**
   ...
   * **必須**
      ...
      これはフォームコンポーネントの種類に依存しますが、このフィールドまたはこのフィールドの特定の部分が必須である/必須であることを示す1つ以上のクリックボックスが用意されています。

   * **必須メッセージ**

      このフィールドが必須であることをユーザーに通知するメッセージ。必須フィールドには、アスタリスクとアスタリスクのフラグも設定されます。

   * **制約**

      選択できる制約は、フォームコンポーネントのタイプによって異なります。

   * **制約メッセージ**

      何が必要かをユーザーに知らせるメッセージ。
...
* **アドバンス**
   ...
   * **アクション**
タイプフォームにはアクションが必要です。アクションは、ユーザーが送信したデータを使用して実行するためにトリガーされる操作を定義します（HTMLのaction=と同様）。 対応する 
**アクションの設定**.

      標準のAEMインストールには、様々なアクションタイプが含まれています。
   ...
   * **読み込み元パス**
      ...
      定義済みの値をフォームフィールドに読み込むために使用されるノードプロパティへのパス。
   ...
   * **アクションの設定**
      ...
      「**アクションの設定**」で使用できるオプションは、選択した「**アクションタイプ**」によって異なります。
      ...
      * **コンテンツを作成**
         ...
         * **リソースタイプ**
            ...
            設定した場合、各コメントに`sling:resourceType`と追加されます。
      ...
      * **アカウントの作成と更新**
         ...
         * **データを表示...**
            ...
            このボタンをクリックして、バルクエディターのフォームの結果に関する情報にアクセスします。ここから、`.tsv`（タブ区切り）ファイルに情報を書き出すことができます（例えば、Excelスプレッドシートで使用）。
      * **メール**
      ...
      * **コンテンツを格納**
         ...
         * **データを表示...**
            ...
            このボタンをクリックして、バルクエディターのフォームの結果に関する情報にアクセスします。ここから、情報を.tsv（タブ区切り）ファイルに書き出すことができます（例えば、Excelスプレッドシートで使用）。
      * **コンテンツをアップロードデータと共に保存**
