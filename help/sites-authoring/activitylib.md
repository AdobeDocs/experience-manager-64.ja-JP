...
>[!NOTE]
>
>Adobe Targetでキャンペーンを作成すると、という名前のプロパティが割り当てられます。 `thirdPartyId t`各キャンペーンに対して Adobe Target でキャンペーンを削除しても、`thirdPartyId` は削除されません。`thirdPartyId` を異なるタイプ（AB、XT）のキャンペーンに再利用することはできません。また、手動で削除することはできません。この問題を回避するには、各キャンペーンに一意の名前を付けてください。そうすれば、キャンペーン名を異なるキャンペーンタイプで再利用できなくなります。
>
