...
* 以下を使用： **テンプレート** コンソール。 これは、 **一般** セクション **ツール** コンソール。
   ...
   * または直接アクセス： [http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf](http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf)
...
* [追加のプロパティの定義](#defining-template-properties-template-author) （必要に応じて）テンプレート用
* [テンプレートの編集](#editing-templates-template-authors) を定義するには、次の手順に従います。
...
* [テンプレートの有効化](#enabling-a-template-template-author) （ページの作成時に使用）
* Web サイトの必要なページまたはブランチのために[テンプレートを許可します。](#allowing-a-template-author)
* [テンプレートを公開](#publishing-a-template-template-author) パブリッシュ環境で使用できるようにするには
...
1. 「**作成**」を選択します。確認メッセージが表示されます。[テンプレートの編集](#editing-templates-template-authors)を開始するには「**開く**」、テンプレートコンソールに戻るには「**完了**」を選択します。
...
* 説明
   ...
   * 説明（オプション）テンプレートとその用途に関する詳細情報を提供します。これは、例えば **ページを作成** ウィザード。
...
* [構造](#editing-a-template-structure-template-author)
   ...
   コンポーネントがロックされている場合は、コンテンツを追加できます。コンテンツはページ作成者が編集できません。 コンポーネントのロックを解除して、 [初期コンテンツ](#editing-a-template-initial-content-author).
...
* [レイアウト](#editing-a-template-layout-template-author)
   ...
   ここでは、必要なデバイスの形式に合わせたテンプレートのレイアウトを事前定義できます。テンプレートオーサリングの&#x200B;**レイアウト**&#x200B;モードには、ページオーサリングの&#x200B;**[レイアウト](/help/sites-authoring/responsive-layout.md#defining-layouts-layout-mode)**&#x200B;モードと同じ機能があります。
...
* **コンポーネントを追加**
   ...
   * コンポーネントがロック解除されたときに表示するマーカー
   ...
   >[!NOTE]
   >
   >まったく同じではありませんが、テンプレートにコンポーネントやアセットを追加する操作は、 [ページオーサリング](/help/sites-authoring/editing-content.md).
...
* **ポリシーおよびプロパティ（一般）**
   ...
   **ポリシーの選択**&#x200B;ドロップダウンで選択された既存のポリシーは、ドロップダウンの横にあるコピーボタンを使用して、新規ポリシーにコピーできます。新しいタイトルを「**ポリシーのタイトル**」フィールドに入力します。デフォルトで、コピーされたポリシーは「**X のコピー**」というタイトルになります。ここで X はコピーされたポリシーのタイトルです。
   ...
   >[!NOTE]
   >
   >同じタイプの複数のコンポーネントが初期コンテンツとして追加される場合は、同じポリシーがすべてのコンポーネントに適用されます。これは、静的テンプレートの&#x200B;**[デザインモード](/help/sites-authoring/default-components-designmode.md)**&#x200B;と同じ制限を反映します。
   ...
   *機能*
   ...
   >[!CAUTION]
   >
   >AEM のトリミング比率は、**高さ/幅**&#x200B;で定義されます。これは、通常の高さ／幅の定義とは異なり、レガシー互換性のための設定です。ページオーサリングユーザーは、**名前**&#x200B;を明確に定義していれば、これが UI に表示されるので、違いに気が付きません。
...
1. この **ページデザイン** ダイアログが開き、次の 2 つのセクションに分割されます。
...
1. [独自のフォルダーの作成](#creating-a-template-folder-admin) サイト固有のテンプレート用に作成されます。
1. [テンプレートを公開](#publishing-a-template-template-author) から **テンプレート** コンソール。
