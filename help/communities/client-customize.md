...
>[!CAUTION]
>
>Any CSS class name that is prefixed with **scf-js-&amp;ast;** has a specific use in javascript code. これらのクラスは、コンポーネントの状態に影響を与え（例えば、非表示から表示に切り替える）、上書きも削除もしないでください。
>
...
1. 拡張コンポーネントのJavaScriptまたは開始を最初からコピーする
1. メソッドの拡張
