...
* **[!UICONTROL 割り当て追加]**
担当者 [プルダウンメニューを使用して、](#members-group) メンバー(学習者として登録するユーザーとユーザーグループ（太字で表示）から選択します。メンバーがコミュニティサイトにサインインすると、登録されているイネーブルメントリソース（および学習パス）が[割り当て](functions.md#assignments-function)ページに表示されます。
...
* **[!UICONTROL タグ]**
   ...
   （*オプションの*）学習パスを1つ以上のカタログに関連付ける1つ以上のタグを選択できます。 [実施可能リソースのタグ付け](tag-resources.md)を参照してください。
...
ごみ箱アイコンを選択すると、有効化リソースは確認後`Delete`dになります。
