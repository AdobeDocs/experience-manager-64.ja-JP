...
これは、Apps 6.1 Feature Pack 2で利用できる機能で、AEM内で新しいアプリを作成する際に既存のアプリテンプレートを簡単に利用できます。
...
1. 新しいアプリを作成すると、アプリカタログに次の新しいアプリが表示されます。
...
>[!NOTE]
>
>Geometrixx Outdoors 参照アプリパッケージを AEM からアンインストールした場合、そのテンプレートに基づいて作成したアプリは機能しなくなります。Geometrixx Outdoorsアプリは削除できますが、他のモバイルアプリケーションで使用する場合は、アプリテンプレートを残しておく必要があります。
...
* 次を使用してリストを作成：カルーセルに含めるページを指定するために使用するメソッドです。 ページリストのビルドを参照してください。
