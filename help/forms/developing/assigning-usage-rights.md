...
>[!NOTE]
>
>Java APIの一部である `applyUsageRights` メソッドを使用してPDFドキュメントに使用権限を適用する場合、 `isModeFinal` オブジェクトの `ReaderExtensionsOptionSpec` パラメータをに設定できま `false`す。 その結果、フォーム処理済みカウンターが更新されず、パフォーマンスが向上します。 フォーム処理済みカウンターの更新を気にしない場合は、この `isModeFinal` パラメーターをに設定することをお勧めし `false`ます。
