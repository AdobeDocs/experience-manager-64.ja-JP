...
>[!NOTE]
>
>`BLOB`オブジェクトを入力する場合は、`setBinaryData`メソッドを呼び出して30 MBを超えないようにすることをお勧めします。 それ以外の場合は、`OutOfMemory`*例外が発生する可能性があります。*
...
1. JDK 1.6以降をインストールします。
   ...
   * JRE binデ追加ィレクトリをクラスパスに格納します。 このビンは、[*JDK_INSTALL_LOCATION*]/jreディレクトリにあります。
