...
ユーザーはフォームに値を入力し、「Calculate」ボタンをクリックして結果を表示します。 次のプロセスでは、ユーザーがデータを計算できるようにするアプリケーションの例を説明します。
...
フォームデザインスクリプトの使用方法を説明するために、この節では、サーバー上で実行するように設定されたスクリプトを含むシンプルなインタラクティブフォームについて調べます。 次の図に、ユーザーが最初の2つのフィールドに入力した値を加算し、3番目のフィールドに結果を表示するスクリプトを含むフォームデザインを示します。
