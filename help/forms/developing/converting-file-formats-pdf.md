---
title: ファイル形式とPDF間の変換
seo-title: ファイル形式とPDF間の変換
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>アプリケーション固有のダイアログボックスに対する応答方法を指定します。 </p><p>この情報を含むファイルはappmonです。<i>[appname]</i>.dialog.<i>[locale]</i>.xml（appmon.word.en_US.xmlなど）</p></td> 
...
* adobe-livecycle-native-jboss-x86_win32.ear > adobe-Native2PDFSvc.war\WEB-INF\lib > adobe-native.jar > Native2PDFSvc-native.jar\bin > adobe-appmondata.jar\com\adobe\appmon。 adobe-livecycle-native-jboss-x86_win32.earファイルは、*[AEM forms install directory]\*configurationManagerの書き出しフォルダーにあります。 (AEM Formsを別のJ2EEアプリケーションサーバーにデプロイする場合は、adobe-livecycle-native-jboss-x86_win32.earファイルを、ご使用のJ2EEアプリケーションサーバーに対応するEARファイルに置き換えます)。
* adobe-generatepdf-dsc.jar/adobe-appmondata.jar\com\adobe\appmon （adobe-appmondata.jarファイルはadobe-generatepdf-dsc.jarファイル内にあります）。 adobe-generatepdf-dsc.jarファイルは、*[AEM forms installディレクトリ]*\deployフォルダーにあります。
...
スクリプトには、ネイティブアプリケーションのウィンドウ要素間を移動し、それらの要素に対して特定の応答を提供する手順が含まれています。 この情報を含むファイルはappmonです。*[appname]*.script.*[locale]*.xml。appmon.notepad.script.en_US.xmlなどがあります。
...
>[!NOTE]
>
>このコンテキストで、「追加」とは、アプリの内容を意味します。[applicationname].addition.[locale].xmlファイルを参照してください。このようなファイルは、ダイアログXMLファイルへの上書きと追加を指定します。
...
追加のdialogXMLファイルを識別するファイル名はappmonです。*[appname]*.addition.*[locale]*.xml。appmon.excel.addition.en_US.xmlなどがあります。

追加のダイアログXMLファイルの名前は、appmonの形式を使用する必要があります。*[applicationname]*.addition.*[locale]*.xml。 ** applicationnameは、XML設定ファイルおよびスクリプトで使用されるアプリケーション名と完全に一致する必要があります。
...
ネイティブ環境の実行可能ファイルの場所を指定するアプリケーション変数を作成します。 変数は&#x200B;*[applicationname]*_PATHの形式を使用する必要があります。*applicationname*&#x200B;は、XML設定ファイルおよびスクリプトで使用されるアプリケーション名と完全に一致し、パスに重複ーの引用符で囲まれた実行ファイルのパスが含まれます。 このような環境変数の例は`Photoshop_PATH`です。
