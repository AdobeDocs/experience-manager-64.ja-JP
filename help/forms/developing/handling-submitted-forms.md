...
1. フォームデータの取得
   ...
   * コンストラクタを使用して `RenderOptionsSpec` オブジェクトを作成します。オブジェクトのメソッドを呼び出し、ロ `RenderOptionsSpec` ケール値を指 `setLocale` 定する文字列値を渡して、ロケール値を設定します。
   >[!NOTE]
   >
   >オブジェクトのメソッドを呼び出して渡し、呼び出して渡すことによって、送信されたPDFコンテンツからXDPまたはXMLデータを作成す `RenderOptionsSpec` るようFormsサービスに指 `setPDF2XDP` 示することがで `true` きま `setXMLData``true`す。 その後、オブジェクトの `FormsResult` メソッドを呼 `getOutputXML` び出して、XDP/XMLデータに対応するXMLデータを取得できます。 (オブ `FormsResult` ジェクトは、次のサブステッ `processFormSubmission`プで説明する*メソッドによって返されます。)*
