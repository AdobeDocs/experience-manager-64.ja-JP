...
次の図に、Webブラウザーに表示されたインタラクティブフォーム `HandleData` からJavaサーブレット名に送信されるデータを示します。
...
送信データのコンテンツタイプに応じて、送信されたXMLデータから個々のフォームフィールド値を抽出したり、送信されたPDFデータをPDFファイルとして保存したり（または別のサービスに送信したり）できます。 個々のフォームフィールドを抽出するには、送信されたXMLデータをXMLデータソースに変換し、クラスを使用してXMLデータソースの値を取得し `org.w3c.dom` ます。
...
1. フォームデータの取得
   ...
   * コンストラクタを使用して `RenderOptionsSpec` オブジェクトを作成します。オブジェクトのメソッドを呼び出し、ロケール値を指定する文字列値を渡すことで、 `RenderOptionsSpec``setLocale` ロケール値を設定します。
   ...
   >[!NOTE]
   >
   >オブジェクトの `RenderOptionsSpec` メソッドを呼び出して渡し、呼び出して渡すことで、送信されたPDFコンテンツからXDPまたはXMLデータを作成するようFormsサービスに指示できま `setPDF2XDP``true``setXMLData``true`す。 その後、 `FormsResult` オブジェクトの `getOutputXML` メソッドを呼び出して、XDP/XMLデータに対応するXMLデータを取得できます。 ( `FormsResult` オブジェクトは `processFormSubmission`*メソッドで返され、次のサブステップで説明します)。*
