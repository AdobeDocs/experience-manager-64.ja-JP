...
詳しくは、「[インタラクティブ通信のテキスト](/help/forms/using/texts-interactive-communications.md)のテキストでルールを作成する」を参照してください。

インタラクティブ通信にテキストフラグメントを含め、エージェントUIを使用してインタラクティブ通信を準備すると、受信者の（フォームデータモデル）データが評価され、テキストが米国の受信者にのみ表示されます。
...
>[!NOTE]
>
>既存のアセットに古い（6.2 SP1 CFP 4 より前の）条件式や繰り返し式が含まれている場合は、アセットに古い構文の条件や繰り返しが表示されます。ただし、古い条件や繰り返しも正常に機能します。新しい条件/繰り返し式と古い条件/繰り返し式は互いに互換性があり、新しい式と古い式が入れ子になって混在します。

1. 関連するテキストモジュールで、条件を設定するテキスト部分を選択し、**条件**&#x200B;をタップします。
...
インタラクティブ通信とレターには、動的な情報を含めることができます。例えば、クレジットカードの取引明細書は、レターが生成されるたびに変化します。繰り返しを使用して、テキストドキュメントフラグメント内の動的情報の書式設定と構造化を行うことができます。
...
1. テキストドキュメントフラグメントを保存し、関連するインタラクティブ通信のプレビューを表示します。フォームデータモデル内のデータに応じて、要素に対して繰り返しを適用すると、プレビューで次のようにトランザクションの詳細がレンダリングされます。
...
1. 必要に応じて、取引を選択的にレンダリングする（例えば、50 セントを超える取引額をレンダリングする）ための条件を挿入します。
   ...
   情報（ここでは「トランザクション」）を選択的にレンダリングする必要がない場合は、ダイアログで次の行を削除して、条件を空のままにします。`${}`. 繰り返し式が保存されるのは、繰り返し式ウィンドウが空になっている（繰り返し式が不要で ${} がない）場合、または繰り返し式ウィンドウに有効な繰り返し条件が含まれている場合です。
...
1. 関連するレターをプレビューして、繰り返しに従ってテキストがレンダリングされていることを確認します。次のファイルを使用して DD 要素の値を入力できます。
   ...
   * 関連するデータディクショナリに添付されている XML データファイル。
   ...
   取引の明細とともに静的テキストが繰り返されています。この手順でテキストに適用する繰り返しによって、静的テキストの繰り返しが容易になります。 条件「${DD_creditcard_TransactionAmount > 0.5}」があるので、0.5 米ドルを下回る取引はレターにレンダリングされません。
