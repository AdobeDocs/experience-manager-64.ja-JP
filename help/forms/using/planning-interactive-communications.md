...
各セクションの太字のテキストはスタティックテキストを表します。データベースには、顧客、請求書、および呼び出し表が含まれます。 フォームデータモデルは、これらのテーブルのいずれかのデータを受信することができます。For more information, see [Create form data model](create-form-data-model-tutorial.md).
