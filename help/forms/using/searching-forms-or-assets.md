...
基本検索を実行するには、テキストフィールドに検索クエリを入力し、リターンを押します。ワイルドカード文字(&amp;ast;)を入力して、任意の数の文字と一致させることもできます。
...
* 単語の部分一致はサポートされていません。部分的な文字列を使用して検索するには、&amp;ast；を使用します。ワイルドカード。 ただし、検索クエリが単語全体に一致する場合は、対応するフォームまたはアセットが表示されます。
