...
### テーブルの列幅を設定する {#set-column-width}

テーブルの列幅を設定するには、次の手順を実行します。

1. 内 **[!UICONTROL コンテンツ]** タブで、 **[!UICONTROL テーブル]** コンポーネントをタップし、設定 (![設定](assets/configure-icon.svg))icon.

1. 値のコンマ区切りリストを **[!UICONTROL 列の幅]** 「 」フィールドでは、テーブルの各列の幅を比例して指定します。 例えば、3 つの列を含むテーブルの場合、2、4、6 を **[!UICONTROL 列の幅]** 「 」フィールドでは、列の幅を 1 列目に2/12、2 列目に4/12、3 列目に6/12と設定します。 2/12は、最初の列の幅が、テーブルの幅の 6 分の 1 を表します。 同様に、4/12は 2 番目の列の幅を表の幅の 3 分の 1 に設定し、6/12は 3 番目の列の幅を表の幅の半分に設定します。
...
1. サイドバーでテーブルを選択し、編集ボタン ![edit-button](assets/edit-button.png) をタップします。
...
![headersonthelft](assets/headersontheleft.png)
...
![collassible_column](assets/collapsible_column.png)
