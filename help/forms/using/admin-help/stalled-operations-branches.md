...
**ステータス：** 停止した項目の場合は常に STALLED です。

**エラー：** 問題の簡単な説明。

**プロセス ID :** プロセスがインスタンス化される（つまり、ユーザーまたは自動ステップがプロセスを開始する）ときに、forms ワークフローが割り当てる正の整数。 この ID を使用して、プロセスインスタンスをライフサイクル全体にわたって追跡できます。
