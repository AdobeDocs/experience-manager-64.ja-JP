...
1. ユーザーに適切な業務カレンダーを割り当てる方法を決定します。ユーザーに業務カレンダーを関連付ける方法は 2 つあります。
   ...
   **業務カレンダーキー：** 業務カレンダーキー（User Management で指定した設定）に基づいて、業務カレンダーをユーザーに割り当てることができます。 その後、業務カレンダーキーを forms ワークフローの業務カレンダーにマップします。
...
1. 新しい業務カレンダーを追加するには、 ![bus_cal_plus](assets/bus_cal_plus.png). ドロップダウンリストに、*新しいカレンダー*&#x200B;と表示されます。それを選択して、カレンダーの名前を入力します。

   既存の業務カレンダーを編集するには、ドロップダウンリストからそのカレンダーを選択します。

1. 「デフォルトの非稼働日」で、週末など、週ごとの非稼働日を選択します。
1. [オプション] 「営業時間を使用」を選択し、営業日の開始時間と終了時間を指定します。
...
1. 左側にあるカレンダーで、祝日など、その他の非稼働日をダブルクリックします。過去の日付は選択できません。選択した非稼働日が右側のリストに表示され、1 行に日付が 2 回示されます。左側の日付を選択し、非稼働日の名前または説明を入力します。
   ...
   非営業日をリストから削除するには、 ![bus_cal_trash](assets/bus_cal_trash.png) その日の横に

1. [オプション] このカレンダーを既定のカレンダーにする場合は、[ 既定のカレンダー ] を選択します。 デフォルトのカレンダーは、ユーザー関連イベントのための他のカレンダーマッピングが存在しない場合、またはタイマーイベントや待機サービスのための業務カレンダーが指定されていない場合に使用されます。デフォルトのカレンダーは削除できません。
...
1. 「システムは次を使用します」リストで、「ユーザーマネージャー業務カレンダーキーの解決」を選択します。
...
1. 管理コンソールで、サービス／Forms ワークフロー／業務カレンダーをクリックし、「マッピング」タブをクリックします。
1. 「システムは次を使用します」リストで、「ディレクトリサービスによって定義されたグループ」を選択します。
