...
* オーサーインスタンスと   作成者インスタンスとパブリッシュインスタンス（crx -repositoryバックアップ）。
   ...
   * オーサーインスタンスと  作成者インスタンスと発行インスタンスは同時に開始するように同期されます。バックアップの実行中も作成者インスタンスと発行インスタンスを引き続き使用できますが、バックアップ中にアセットを発行しないで、キャプチャされない変更を防ぐことをお勧めします。 新しいアセットをパブリッシュする前に、オーサーインスタンスとパブリッシュインスタンスの両方のバックアップが終了するまで待機してください。
...
1. コマンドプロンプトで、次のコマンドを 1 行で実行します。
   ...
   * (Windows) `LCBackupMode.cmd enter [-Host=`**number`] [-port=`**`] [-user=`**username`] [-password=`** password `] [-label=`**`] [-timeout=`** password porthname nostname `]`
   * (Linux、UNIX) `LCBackupMode.sh enter [-host=`**`] [-port=`** portnumber `] [-user=`**username`] [-password=`*password* password `] [-label=`**labelname hostname`]`
...
1. 次のコマンドを 1 行で実行します。
   ...
   * (Windows) `LCBackupMode.cmd leaveContinuousCoverage [-Host=`*hostname *`] [-port=`** `] [-user=`*password portnumber *`] [-password=`** password password portnumber `]`
