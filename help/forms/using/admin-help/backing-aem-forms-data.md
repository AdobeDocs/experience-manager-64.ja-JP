...
1. コマンドプロンプトで、次のコマンドを 1 行で実行します。
   ...
   * (Windows) `LCBackupMode.cmd enter [-Host=`**`] [-port=`** number `] [-user=`*username *number`] [-password=`*usernameパスワードpassword relabelname*`] [-label=`**`] [-timeout=`*porthostname* `]`
   * (Linux、UNIX) `LCBackupMode.sh enter [-host=`**`] [-port=`** portnumber `] [-user=`**portnumber`] [-password=`*password username* username `] [-label=`*password username  labelname *`]`
