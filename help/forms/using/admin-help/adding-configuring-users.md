...
MySQL を使用する場合は、ユーザー ID に拡張文字が含まれることがあります。ただし、abcde と âbcdè のような 2 つの文字列を比較すると、同じものと判断されます。例えば、同期を行う場合、データベースに新しいユーザーが追加されていると、同一のユーザー ID を持つユーザーがデータベースに存在するかどうかを調べるために比較が行われます。If user *abcde* already exists in the database when the new user *âbcdè * is added, the comparison cannot distinguish between the two names. データベースには既に同じ名前のユーザーが追加されているものと見なされ、新しいユーザーは無視されて追加されません。
