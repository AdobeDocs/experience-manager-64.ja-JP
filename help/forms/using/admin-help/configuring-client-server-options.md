...
**ベース URL :** サーバー名とポートを含む、ベース Document Security URL です。 ベースに情報が追加されて、接続 URL が作成されます。例えば、/edc/Main.do が追加されて Web ページへのアクセスが行われます。ユーザーは、この URL を使用して、外部ユーザー登録の招待に応答することもできます。
...
**デフォルトのオフラインリース期間：** ユーザーが保護されたドキュメントをオフラインで使用できるデフォルトの長さです。 この設定により、ポリシーを作成するときに自動オフラインリース期間設定の初期値が決まります（ポリシーの作成および編集を参照）。リース期間が終了した後、ドキュメントを引き続き使用するには、受信者がそのドキュメントを再び同期する必要があります。
...
**デフォルトのオフライン同期期間：** ドキュメントが最初に保護されてから、任意のドキュメントをオフラインで使用できる最大時間です。

**クライアントセッションタイムアウト：** クライアントアプリケーションを通じてログインしたユーザーが Document Security とやり取りしない場合に Document Security が切断されるまでの時間（分単位）です。

**匿名ユーザーのアクセスを許可：** 匿名ユーザーがポリシーで保護されたドキュメントを開くことを許可する共有ポリシーおよび個人用ポリシーを作成する機能を有効にするには、このオプションを選択します。 （アカウントを持たないユーザーでもドキュメントにアクセスできますが、Document Security にログインしたり、他のポリシーで保護されたドキュメントを使用したりすることはできません）。

**バージョン 7 クライアントへのアクセスを無効にする：** ユーザーがAcrobatまたはReader7.0 を使用してサーバーに接続できるかどうかを指定します。 このオプションが選択されている場合、ユーザーは Acrobat または Reader 8.0 以降を使用して、PDF ドキュメントで Document Security の処理を実行する必要があります。ポリシーで保護されたドキュメントを開く際に、Acrobat または Reader 8.0 以降を認証モードで実行することがポリシーによって指定されている場合は、Acrobat または Reader 7 へのアクセスを無効にする必要があります（ユーザーおよびグループのドキュメント権限の指定を参照）。
...
**ユーザー名パスワード認証を許可：** クライアントアプリケーションがサーバーへの接続時にユーザー名/パスワード認証を使用できるようにするには、このオプションを選択します。

**Kerberos 認証を許可：** このオプションを選択すると、クライアントアプリケーションがサーバーに接続する際に Kerberos 認証を使用できるようになります。

**クライアント証明書認証を許可：** クライアントアプリケーションがサーバーへの接続時に証明書認証を使用できるようにするには、このオプションを選択します。

**拡張認証を許可** 拡張認証を有効にする場合に選択し、拡張認証ランディング URL を入力します。
...
**拡張HTMLの優先認証制御幅** ユーザー資格情報を入力するためにAcrobatで開く拡張認証ダイアログの幅を指定します。

**拡張HTMLの優先認証制御の高さ** Acrobatで開き、ユーザー資格情報を入力するための拡張認証ダイアログの高さを指定します。
...
**クライアント資格情報のキャッシュを有効にする：** ユーザーが自分の資格情報（ユーザー名とパスワード）をキャッシュできるようにするには、このオプションを選択します。 ユーザーの資格情報がキャッシュされていると、ドキュメントを開くたびに、または Adobe Acrobat のセキュリティポリシーを管理ページで「更新」ボタンをクリックするときに、資格情報を入力する必要がありません。ユーザーに資格情報の再入力を求める日までの日数を指定できます。日数を 0 に設定すると、資格情報は無期限にキャッシュされます。
...
1. 内 `AllowedUrls` ノードに次の行を追加します。 `<entry key="sso-l" value="/ssoexample/login.jsp"/> <entry key="sso-s" value="/ssoexample"/> <entry key="sso-o" value="/ssoexample/logout.jsp"/>`
...
**低解像度で印刷** 受信者は、低解像度オプションが指定されたポリシーで保護されたドキュメントを印刷します。

**高解像度印刷：** 受信者は、ポリシーで保護されたドキュメントを高解像度オプションで印刷します。

**ドキュメントに注釈を追加：** 受信者が注釈ドキュメントにPDFを追加します。

**ドキュメントを失効：** ユーザーまたは管理者が、ポリシーで保護されたドキュメントへのアクセスを無効にしました。

**ドキュメントの失効取消：** ユーザーまたは管理者が、ポリシーで保護されたドキュメントへのアクセス権を回復します。

**フォーム入力：** 受信者が、入力可能なフォームであるPDFドキュメントに情報を入力します。

**削除されたポリシー：** 発行者がドキュメントからポリシーを削除し、セキュリティ保護を取り消しました。

**ドキュメント失効 URL の変更：** API レベルからの呼び出しで、失効したドキュメントに代わる新しいドキュメントにアクセスするために指定された失効 URL が変更されます。
...
**ドキュメントのポリシーを切り替え：** ユーザーまたは管理者が、ドキュメントに添付されているポリシーを切り替えました。

**ドキュメントの公開名：** ドキュメント名とライセンスが既存のドキュメントと同一の新しいドキュメントがサーバーに登録され、そのドキュメントは親子関係を持ちません。 このイベントは、AEM Forms SDK を使用してトリガーされます。

**ドキュメントを繰り返し：** ドキュメント名とライセンスが既存のドキュメントと同一の新しいドキュメントがサーバーに登録され、そのドキュメントは親子関係を持ちます。 このイベントは、AEM Forms SDK を使用してトリガーされます。

**ポリシーイベント**

**作成されたポリシー：** ユーザーまたは管理者がポリシーを作成します。

**有効なポリシー：** 管理者がポリシーを使用可能にします。

**変更されたポリシー：** ユーザーまたは管理者がポリシーを変更しました。

**無効なポリシー：** 管理者がポリシーを使用不可にしました。

**削除されたポリシー：** ユーザーまたは管理者がポリシーを削除しました。

**ポリシー所有者の変更：** API レベルからの呼び出しで、ポリシーの所有者が変更されます。
...
**招待ユーザ：** Document Security はユーザーに登録を促します。

**アクティベートされたユーザー：** 外部ユーザーは、アクティベーション電子メールに記載されている URL を使用してアカウントをアクティベートするか、管理者がアカウントを有効にします。

**パスワードの変更：** 招待ユーザーがパスワードを変更した場合、または管理者がローカルユーザーのパスワードをリセットした場合。

**ログイン失敗：** 管理者またはユーザーによるログイン試行が失敗しました。

**非アクティブなユーザー：** 管理者がローカルユーザーアカウントを無効にしました。

**プロファイルの更新：** 招待されたユーザーが自分の名前、組織名、パスワードを変更しました。

**ロックされたアカウント：** 管理者がアカウントをロックしました。

**ポリシーセットイベント**

**作成されたポリシーセット：** 管理者またはポリシーセットコーディネーターがポリシーセットを作成します。

**削除されたポリシーセット：** 管理者またはポリシーセットコーディネーターがポリシーセットを削除しました。

**変更されたポリシーセット：** 管理者またはポリシーセットコーディネーターがポリシーセットを変更しました。

**システムイベント**

**ディレクトリの同期が完了しました：** この情報は、イベントページからは利用できません。 現在の同期状態、最後の同期時刻など、現在のディレクトリ同期情報は、ドメインの管理ページに表示されます。管理コンソールで、ドメインの管理ページにアクセスするには、設定／User Management／ドメインの管理をクリックします。

**クライアントによるオフラインアクセスの有効化：** ユーザーが、ユーザーのコンピューター上のサーバーに対して保護されたドキュメントへのオフラインアクセスを有効にしました。

**同期されたクライアント** オフラインアクセスを許可するには、クライアントアプリケーションがサーバーと情報を同期する必要があります。

**バージョンの不一致：** サーバーと互換性のないAEM forms SDK のバージョンがサーバーへの接続を試みました。
...
**サーバー構成の変更：** サーバー設定に対する変更が、Web ページを通じて行われるか、config.xml ファイルを読み込んで手動で行われます。 これには次の設定に対する変更が含まれます。ベース URL、セッションタイムアウト、ログインロックアウト、ディレクトリ設定、キーのロールオーバー、外部登録に関する SMTP サーバー設定、透かし設定、表示オプションなど。
...
**認証タイプテキストを選択：** ユーザーに認証タイプを選択するよう指示するために表示されるテキストです。

**証明書テキストを選択：** ユーザーに証明書の種類を選択するよう指示するテキストです。

**証明書を利用できませんエラーテキスト：** 選択した証明書が使用できない場合に表示する最大 512 文字のメッセージ。

**クライアント証明書表示のカスタマイズ**

**信頼できる秘密鍵証明書の発行者のみを表示する：** このオプションを選択すると、クライアントアプリケーションは、信頼するように設定された秘密鍵証明書発行者からの証明書のみをユーザーに表示します（証明書と秘密鍵証明書の管理を参照）。 このオプションの選択を解除すると、ユーザーのシステム上の証明書すべてがユーザーに一覧表示されます。
...
**除外：** 除外するユーザーまたはグループの電子メールアドレスを入力します。 複数のユーザーまたはグループを除外するには、それぞれの電子メールアドレスを別々の行に入力します。特定のドメインに属するすべてのユーザーを除外するには、ワイルドカードとドメイン名を入力します。例えば、example.com ドメイン内のすべてのユーザーを除外するには、&amp;ast;.example.com と入力します。

**包含：** 含めるユーザーまたはグループの電子メールアドレスを入力します。 複数のユーザーまたはグループを対象範囲に含めるには、それぞれの電子メールアドレスを別々の行に入力します。特定のドメインに属するすべてのユーザーを対象範囲に含めるには、ワイルドカードとドメイン名を入力します。例えば、example.com ドメイン内のすべてのユーザーを含めるには、&amp;ast;.example.com と入力します。
...
**SMTP サーバーのソケットのクラス名：** SMTP サーバーのソケットクラス名。 例えば、javax.net.ssl.SSLSocketFactory です。

**E メールのコンテンツタイプ：** text/plain や text/html など、使用可能な MIME タイプです。

**電子メールのエンコード：** 電子メールメッセージの送信時に使用するエンコード形式。 任意のエンコードを指定できます。例えば、Unicode の UTF-8 や Latin の ISO-8859-1 です。デフォルトは UTF-8 です。

**リダイレクトメールアドレス：** この設定の電子メールアドレスを指定すると、指定されたアドレスに新しい招待メールが送信されます。 この設定は、テストを目的とする場合に役立ちます。
...
1. 管理コンソールを開きます。
   ...
   管理コンソールのデフォルトの場所は次のとおりです。 `https://<server>:<port>/adminui`.
...
1. 招待ユーザーの登録ですべての設定項目を指定し、「OK」をクリックします。
   ...
   >[!NOTE]
   >
   >ユーザー登録の招待状を送信する SMTP サーバーとしてMicrosoft Office 365 を使用している場合は、次の設定を使用します。
   >
...
**送信者：** 招待メールの送信元の電子メールアドレス。 送信元メールアドレスのデフォルトの形式は、 postmaster@です。[your_installation_domain].com

**件名：** 招待用電子メールメッセージのデフォルトの件名です。

**タイムアウト：** 外部ユーザーが登録しない場合に登録招待が期限切れになるまでの日数です。 デフォルト値は 30 日です。

**メッセージ：** 登録するユーザーを招待するメッセージの本文に表示されるテキストです。
...
**送信者：** アクティベーション電子メールの送信元の電子メールアドレス。 この電子メールアドレスに、登録者の電子メールホストから配信失敗通知が届きます。また、受信者が登録電子メールへの応答として送信するメッセージも届きます。送信元メールアドレスのデフォルトの形式は、 postmaster@です。[your_installation_domain].com

**件名：** アクティベーション電子メールメッセージのデフォルトの件名です。

**タイムアウト：** ユーザーがアカウントをアクティブ化しない場合にアクティベーションへの招待が期限切れになるまでの日数です。 デフォルト値は 30 日です。

**メッセージ：** 受信者のユーザーアカウントを有効にする必要があることを示すメッセージをメッセージ本文に表示するテキストです。 それ以外の情報として、新しいパスワードを取得するために管理者に問い合わせる方法なども含めるとよいでしょう。
...
**送信者：** パスワードリセット用の電子メールの送信元の電子メールアドレス。 送信元メールアドレスのデフォルトの形式は、 postmaster@です。[your_installation_domain].com

**件名：** リセット用電子メールメッセージのデフォルトの件名です。
...
**表示される「ユーザーとグループ」タブ** ユーザーの検索結果に表示するユーザーとグループを制御するために使用します。 スーパーユーザーまたはポリシーセット管理者は、User Management で作成されたドメインを選択し、ポリシーセットごとに表示されるユーザーおよびグループに追加する必要があります。このリストはポリシーセットコーディネーターによって参照されるものであり、ポリシーに追加するユーザーを選択するときにポリシーセットコーディネーターが参照できるドメインを制限するために使用されます。
...
1. 内 `ServerSettings` ノードに次のエントリを追加し、ファイルを保存します。 `<entry key="maximumSizeOfWatermarkElement" value="max filesize in KB"/> <entry key="maximumWatermarkElementsPerWatermark" value="max elements"/>`

   最初のエントリ、「最大ファイルサイズ」は、透かし要素に使用できる最大ファイルサイズ（KB 単位）です。 デフォルトは 100 KB です。
...
1. すべての外部リンクを無効にするには、 `DisplaySettings` ノードに次のエントリを追加し、ファイルを保存します。 `<entry key="ExternalLinksAllowed" value="false"/>`
...
1. ServerSettings ノードで、 `DisableGlobalOfflineSynchronizationData`プロパティを `true`.
