...
**** ベースURL:サーバー名とポートを含む、ベースDocument Security URL。 ベースに情報が追加されて、接続 URL が作成されます。例えば、/edc/Main.do が追加されて Web ページへのアクセスが行われます。ユーザーは、この URL を使用して、外部ユーザー登録の招待に応答することもできます。
...
**** クライアントセッションタイムアウト：クライアントアプリケーションを使用してログインしたユーザーがDocument Securityとやり取りしない場合に、Document Securityが切断されるまでの時間（分単位）です。
...
**[ドキュメントごとにオフラインアクセスを許可する** ]ドキュメントごとにオフラインアクセスを指定する場合は、このオプションを選択します。 この設定が有効な場合、ユーザーは少なくとも 1 回オンラインで開いたドキュメントのみに対し、オフラインアクセス権があります。
...
**拡張認証を許可** ：拡張認証を有効にし、拡張認証のランディングURLを入力します。
...
**拡張認証用の推奨HTML制御幅** Acrobatで開く拡張認証ダイアログの幅を指定し、ユーザーの資格情報を入力します。

**拡張認証の推奨HTML制御の高さ** Acrobatで開き、ユーザーの資格情報を入力するための拡張認証ダイアログの高さを指定します。

*****注意&#x200B;*:このダイアログボックスの幅と高さの制限は次のとおりです。幅：最小= 400、最大= 900

高さ：最小 = 450、最大 = 800

**** クライアント資格情報キャッシュを有効にする：ユーザーが資格情報（ユーザー名とパスワード）をキャッシュできるようにする場合は、このオプションを選択します。 ユーザーの資格情報がキャッシュされていると、ドキュメントを開くたびに、または Adobe Acrobat のセキュリティポリシーを管理ページで「更新」ボタンをクリックするときに、資格情報を入力する必要がありません。ユーザーに資格情報の再入力を求める日までの日数を指定できます。日数を 0 に設定すると、資格情報は無期限にキャッシュされます。
...
**低解像度で印刷** 受信者は、ポリシーで保護されたドキュメントを低解像度で印刷します。

**** 高解像度で印刷：受信者がポリシーで保護されたドキュメントを高解像度で印刷しました。

**** ドキュメントに注釈を追加：受信者がPDFドキュメントに注釈を追加しました。

**** ドキュメントの失効：ユーザーまたは管理者が、ポリシーで保護されたドキュメントへのアクセス権限を失効させました。

**** ドキュメントの失効取り消し：ユーザーまたは管理者が、ポリシーで保護されたドキュメントへのアクセス権を復元しました。

**** フォーム入力：受信者が入力可能なフォームであるPDFドキュメントに情報を入力します。

**** 削除されたポリシー：発行者がドキュメントからポリシーを削除し、セキュリティ保護を取り消しました。

**** ドキュメント失効URLの変更：APIレベルからの呼び出しによって、失効したドキュメントに代わる新しいドキュメントにアクセスするために指定された失効URLが変更されます。

**** ドキュメントの変更：受信者がポリシーで保護されたドキュメントのコンテンツを変更しました。

**** ドキュメントに署名：受信者がドキュメントに署名します。

**** 新規ドキュメントの保護：ユーザーがドキュメントを保護するためのポリシーを適用した場合。

**** ドキュメントのポリシーの切り替え：ユーザーまたは管理者が、ドキュメントに添付されているポリシーを切り替えました。

**** ドキュメントの発行：ドキュメント名とライセンスが既存のドキュメントと同じである新しいドキュメントがサーバーに登録され、ドキュメントに親子関係が設定されていません。 このイベントは、AEM Forms SDK を使用してトリガーされます。

**** ドキュメントの反復：ドキュメント名とライセンスが既存のドキュメントと同じである新しいドキュメントがサーバーに登録され、ドキュメントに親子関係が設定されます。 このイベントは、AEM Forms SDK を使用してトリガーされます。
...
**** 無効なポリシー：管理者がポリシーを使用できなくしました。

**** ポリシーの削除：ユーザーまたは管理者がポリシーを削除しました。

**** ポリシー所有者の変更：APIレベルからの呼び出しによって、ポリシーの所有者が変更されます。

**ユーザーイベント**

**** ユーザーの削除：管理者がユーザーアカウントを削除しました。

**** 招待ユーザーの登録：外部ユーザーがDocument Securityに登録しました。

**** ログイン成功：管理者またはユーザーによるログインが正常に試行されました。

**** 招待ユーザー：Document Securityはユーザーに登録を勧めました。

**** アクティベートされたユーザー：外部ユーザーがアクティベート用の電子メールのURLを使用して自分のアカウントをアクティベートするか、管理者がアカウントを有効にします。

**** パスワードの変更：招待ユーザーがパスワードを変更したり、管理者がローカルユーザーのパスワードをリセットしたりします。

**** ログイン失敗：管理者またはユーザーによるログイン試行が失敗しました。

**** 非アクティブなユーザー：管理者がローカルユーザーアカウントを無効にしました。

**** プロファイルの更新：招待ユーザーが名前、組織名およびパスワードを変更しました。

**** アカウントロック：管理者がアカウントをロックしました。
...
**** クライアントのオフラインアクセスを有効にする：ユーザーが、ユーザーのコンピューター上のサーバーに対して保護されたドキュメントへのオフラインアクセスを有効にしました。

**同期されたクライアント** ・クライアント・アプリケーションは、オフライン・アクセスを許可するために、情報をサーバと同期する必要があります。
...
**** サーバー設定の変更：サーバー設定に対する変更は、Webページを使用して行うか、config.xmlファイルを読み込んで手動で行います。 これには次の設定に対する変更が含まれます。ベース URL、セッションタイムアウト、ログインロックアウト、ディレクトリ設定、キーのロールオーバー、外部登録に関する SMTP サーバー設定、透かし設定、表示オプションなど。
...
**** ウェルカムテキスト：「ユーザー名とパスワードを使用してログインしてください」など、ウェルカムメッセージのテキスト。 ウェルカムメッセージのテキストには、Document Security へのログイン方法と、管理者または組織内のサポート担当者への連絡方法が記載されている必要があります。例えば、外部ユーザーがパスワードを忘れた場合や、登録方法やログイン方法に関して質問がある場合は、管理者に問い合わせる必要があります。このウェルカムテキストでは最大 512 文字を使用できます。

**** User Name Text:ユーザー名ボックスのテキストラベルです。
...
**** ChooseAuthentication Type Text:ユーザーに認証タイプを選択するよう指示するテキスト。

**** 証明書テキストの選択：ユーザーに証明書の種類を選択するよう指示するテキストです。

**** Certificates Not Available Error Text:選択した証明書が使用できない場合に表示する512文字までのメッセージです。

**クライアント証明書表示のカスタマイズ**

**** Display Trusted Credential Issuers:このオプションを選択すると、クライアントアプリケーションによって、AEM formsが信頼するように設定されている秘密鍵証明書発行者からの証明書のみがユーザーに提示されます（「証明書と秘密鍵証明書の管理」を参照）。このオプションの選択を解除すると、ユーザーのシステム上の証明書すべてがユーザーに一覧表示されます。
...
**** SMTPサーバーソケットクラス名：SMTPサーバーのソケットクラス名です。 例えば、javax.net.ssl.SSLSocketFactory です。

**** 電子メールコンテンツタイプ：text/plainやtext/htmlのような、受け入れられるMIMEタイプ。

**** 電子メールエンコーディング：電子メールメッセージの送信時に使用するエンコード形式。 任意のエンコードを指定できます。例えば、Unicode の UTF-8 や Latin の ISO-8859-1 です。デフォルトは UTF-8 です。
...
**** SMTPサーバーにSSLを使用：SMTPサーバーに対してSSLを有効にする場合は、このオプションを選択します。
...
**** 開始日：招待用電子メールの送信元の電子メールアドレス。 The default format of the From email address is postmaster@[your_installation_domain].com.
...
**** 開始日：アクティベート用の電子メールの送信元の電子メールアドレスです。 この電子メールアドレスに、登録者の電子メールホストから配信失敗通知が届きます。また、受信者が登録電子メールへの応答として送信するメッセージも届きます。The default format of the From email address is postmaster@[your_installation_domain].com.

**** 件名：アクティベートの電子メールメッセージのデフォルトの件名です。

**** タイムアウト：ユーザーがアカウントをアクティベートしない場合にアクティベートの招待が期限切れになる日数です。 デフォルト値は 30 日です。

**** メッセージ：受信者のユーザーアカウントをアクティブにする必要があることを示すメッセージの本文に表示されるテキストです。 それ以外の情報として、新しいパスワードを取得するために管理者に問い合わせる方法なども含めるとよいでしょう。
...
**** 開始日：パスワードリセット用の電子メールの送信元の電子メールアドレス。 The default format of the From email address is postmaster@[your_installation_domain].com.
...
**** 「ポリシーを作成」タブ：カスタムポリシーを作成するためのユーザー権限を設定するために使用します。
...
1. エディターで設定ファイルを開いて、`PolicyServer` ノードを見つけます。`ClientVersionRules` ノードを `PolicyServer` ノードのすぐ下の子として追加します（存在しない場合）。
   ...
   * Sun Solaris
   * HP-UX
   `SDKVersions` は、クライアントアプリケーションで使用される Document Security C++ Client API のバージョンを指定します。For example, `"8.2"`.
...
1. In the `ServerSettings` node, add the following entries and then save the file: `<entry key="maximumSizeOfWatermarkElement" value="max filesize in KB"/> <entry key="maximumWatermarkElementsPerWatermark" value="max elements"/>`
   ...
   最初のエントリ、*max file size *は、PDF透かし要素に許可される最大ファイルサイズ(KB)です。 デフォルトは 100 KB です。
