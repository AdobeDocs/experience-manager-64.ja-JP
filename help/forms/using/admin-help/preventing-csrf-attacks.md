...
**ホワイトリスト登録済みURI:** URIはリクエスト対象のformsサーバー上のリソースを識別します（例：/adminui、/contentspace）。 一部のリソースでは、外部のサイトからリクエストがアプリケーションにアクセスすることができます。これらのリソースは、許可されたURIと見なされます。 formsサーバーは、許可されたURIからリファラーのチェックを実行しません。
