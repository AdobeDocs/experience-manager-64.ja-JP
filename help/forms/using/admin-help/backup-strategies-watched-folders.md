...
次の表に、プロセス全体での 5 つのサンプルファイル (file1、file2、file3、file4、file5) のファイル操作を示します。 この表では、x 軸は Time 1 や T1 などの時刻を表し、y 軸は Input などの監視フォルダー階層内のフォルダーを表しています。
...
* 使用可能な監視フォルダーのバックアップがジョブの処理にかかる時間より古い場合は、新しい監視フォルダーを作成し、自動的に入力フォルダーにファイルを配置することを許可する必要があります。
* 使用可能な最新のバックアップが十分に最新でない場合、バックアップ時間はファイルの処理に要する時間より短く、監視フォルダーが復元されると、ファイルは次のいずれかの段階で操作されます。
   ...
   * **ステージ 5:** 返された結果
   ...
   ファイルがステージ 1 にある場合、ファイルは操作されます。 ファイルがステージ 2 または 3 にある場合は、操作を再実行するために入力フォルダーに配置します。
