...
**プロセス ID :** プロセスがインスタンス化される（つまり、ユーザーまたは自動ステップがプロセスを開始する）際に、forms ワークフローが割り当てる識別子。 この ID を使用して、プロセスインスタンスをライフサイクル全体にわたって追跡できます。
...
**完了：** プロセスインスタンス内のすべてのブランチと操作が完了しました。 「完了」は、プロセスインスタンスの最後のステータスです。

**完了中：** プロセスインスタンスのステータスが「完了」に変わろうとしています。
...
**休止：** プロセスインスタンスは、管理者またはプロセス内のステップによって中断されています。 ステータスが変わるまで、操作は一切行われません。
...
**休止解除中：** SUSPENDED の後、ステータスが RUNNING に変わろうとしています。
...
**ステータス：** 操作が正常に実行されているか、または停止しているかを示します。 （プロセスインスタンスのステータスについてを参照）。

**支店名：** Workbench で定義されるブランチの名前。
...
**プロセス ID :** プロセスがインスタンス化される（つまり、ユーザーまたは自動ステップがプロセスを開始する）ときに、forms ワークフローが割り当てる正の整数です。 この ID を使用して、プロセスインスタンスをライフサイクル全体にわたって追跡できます。
