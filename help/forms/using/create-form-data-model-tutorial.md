...
* [フォームデータモデルを作成する](#step-create-form-data-model)
...
**呼び出し**&#x200B;テーブルには、呼び出し日、呼び出し時間、呼び出し回数、呼び出し時間、呼び出し料金などの呼び出しの詳細が含まれます。 **customer**&#x200B;テーブルは、「モバイル番号(mobilenum)」フィールドを使用して呼び出しテーブルにリンクされています。 **customer**&#x200B;テーブルにリストされているモバイル番号ごとに、**calls**&#x200B;テーブルに複数のレコードがあります。 例えば、**calls**&#x200B;テーブルを参照して、**1457892541**&#x200B;モバイル番号の呼び出しの詳細を取得できます。
