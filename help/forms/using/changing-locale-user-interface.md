...
上記の手順を実行する前に、「[AEM Forms Workspaceのカスタマイズの一般的な手順](/help/forms/using/generic-steps-html-workspace-customization.md)」に示す手順に従ってください。
...
1. CRXDE Lite にログインします。
   ...
   デフォルトのCRXDE LiteURLは`https://[server]:[port]/lc/crx/de/index.jsp`です。
...
AEM Forms workspaceは、カテゴリ、キュー、プロセスのヘッダーを表示するために画像を使用します。 これらのヘッダをローカライズするには、開発パッケージが必要です。開発パッケージの作成について詳しくは、「[AEM Forms Workspaceコードの構築](introduction-customizing-html-workspace.md#building-html-workspace-code)」を参照してください。
...
1. 「 [Workspaceのカスタマイズ](/help/forms/using/introduction-customizing-html-workspace.md) 」の記事に記載されているすべてのセマンティック変更を実行します。
...
日付選択APIをローカライズするには、開発パッケージが必要です。 開発パッケージの作成について詳しくは、「[AEM Forms workspaceコードの構築](introduction-customizing-html-workspace.md#building-html-workspace-code)」を参照してください。
...
1. 日付選択を使用するすべてのファイルに`jquery.ui.datepicker-nw.js`ファイルを含めるには、エイリアス`jqueryuidatepickernw`を使用します。 日付選択は次のファイルで使用されます。
