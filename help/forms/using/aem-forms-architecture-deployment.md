...
**OSGi上のAEM Forms:** OSGi環境上のAEM Formsは、標準のAEM AuthorまたはAEM PublishとAEM Formsパッケージが展開された状態のAEM Authorです。 You can run AEM Forms on OSGi in a [single server environment, Farm, and clustered setups](/help/sites-deploying/recommended-deploys.md). クラスター設定は、AEM作成者インスタンスでのみ使用できます。
