...
1. クエリを作成するには、 **** 追加(+)をクリックします。
   ...
   **フィルターを定義するには**
   ...
   1. クエリに別のフィルターを追加するには、フィルター行の右側にある **追加(+)** をクリックします。
      ...
      クエリからフィルタを削除するには、フィルタ行の右側にある **削除(-)** をクリックします。
...
フィルター **パネルのオプションを使用して** 、 ****![](assets/lc_pr_add_filter.png)lc **_pr_add_filter、lc_edit** lc_edit lc_edit ![](assets/lc_pr_delete_filter.png)****![](assets/lc_pr_edit_filter.png)filter lc_delete pr_delet pr_pr_edit lc_hocクエリのを使用します。
...
1. クエリ結果に対する操作を実行するには、 **レポート** パネルの右上隅にあるオプションを使用します。
...
1. プロセス/タスク検索を実行します。 詳しくは、「プロセス検索クエリ [を実行するには](/help/forms/using/process-reporting/adhoc-queries-in-process-reporting.md#p-to-execute-a-search-query-p)」を参照してください。
