...
1. タスクをクリックしてプロセスを選択します。プロセスに関連付けられているフォームが「フォーム」タブに開きます。
   ...
   プロセス内の各フォームには、一意のURLが割り当てられます。 一意のURLを使用して、特定のプロセスおよびフォームでHTML Workspaceを直接起動できます。 URLの形式はhttps://&lt;server>:&lt;port>/lc/libs/ws/index.html#/startprocess/&lt;ApplicationName>%2F&lt;ProcessName>です。 &lt;ApplicationName>%2F&lt;ProcessName>文字列は、常にURLエンコードされます。 URLの例はhttp://localhost:8080/lc/libs/ws/index.html#/startprocess/MyApplication%2FNewProcessです。 この例のApplicationName%2FProcessName文字列は、URLエンコードされています。
...
1. 「**OK**」をクリックします。メモを含むテキストファイルがフォームに添付されます。メモをクリックして、テキストを直接変更することによって、メモを更新できます。メモの横にある&#x200B;**削除**&#x200B;ボタン![ごみ箱の画像](assets/icondelete.png)をクリックして、メモを削除できます。
...
1. 任意のタブの左下隅にある「**保存**」をクリックします。 フォームが TODO ページのドラフトカテゴリに追加されます。フォームに行った変更がすべて保存されます。
