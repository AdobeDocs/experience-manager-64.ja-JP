...
1. タスクをクリックしてプロセスを選択します。プロセスに関連付けられているフォームが「フォーム」タブに開きます。
   ...
   プロセス内の各フォームには一意の URL が割り当てられます。 固有の URL を使用して、特定のプロセスとフォームでHTMLWorkspace を直接起動できます。 URL の形式はhttps://です。&lt;server>:&lt;port>/lc/libs/ws/index.html#/startprocess/&lt;applicationname>%2F&lt;processname>. この &lt;applicationname>%2F&lt;processname> 文字列は常に URL エンコードされます。 URL の例はhttp://localhost:8080/lc/libs/ws/index.html#/startprocess/MyApplication%2FNewProcessです。 この例の ApplicationName%2FProcessName 文字列は URL エンコードされています。

1. フォームと共に表示される指示に従ってフォームに入力します。必要に応じて、 **最大化** をクリックして、フォームの表示領域を拡大します。
...
1. 「**OK**」をクリックします。メモを含むテキストファイルがフォームに添付されます。メモをクリックして、テキストを直接変更することによって、メモを更新できます。メモを削除するには、 **削除** ボタン ![ごみ箱の画像](assets/icondelete.png) メモの横に
...
1. 「**OK**」をクリックします。ファイルがフォームに添付されます。ファイルを削除するには、 **削除** ボタン ![ごみ箱の画像](assets/icondelete.png) 添付ファイルの横に
...
1. クリック **保存** をクリックします。 フォームが TODO ページのドラフトカテゴリに追加されます。フォームに行った変更がすべて保存されます。
