...
モバイルデバイスへデータをダウンロードすると処理能力に影響を与えるため、デフォルトでは、「添付ファイルの取得」オプションは「オフ」に設定されています。これらの設定を「有効」に変更すると、タスクがサーバーからダウンロードされる際に添付ファイルもデバイスにダウンロードされます。オフラインモードでは、**添付ファイルの取得**&#x200B;オプションをオンに設定した後、デバイスにダウンロードされるすべてのタスクで作業を行うことができます。
...
1. オーサーインスタンスで、**Adobe Experience Manager**/**ツール**/**Forms**/**Formsアプリのオフラインサービスを設定**&#x200B;に移動します。
