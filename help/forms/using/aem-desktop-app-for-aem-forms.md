...
AEMデスクトップアプリケーションは、Microsoft WindowsではWebDAVプロトコルを使用し、Mac OS XではSMB1を使用してAEM Formsサーバーに接続します。 AEM Formsサーバーは、バイナリファイルやその他のアセットをWebDAVまたはSMBクライアントと同期する機能を初期設定で使用できません。 AEM Forms for AEM Desktop Appを有効にするには、次の手順を実行します。
...
1. AEM Forms サーバーを再起動します。再起動後、AEM FormsサーバーはAEMデスクトップアプリケーションに対するコンテンツの受け入れと共有を有効にします。
