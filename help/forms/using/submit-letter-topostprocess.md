...
1. **AEMワークフロー：** AEMワークフローは、送信済みレターの後処理としても使用できます。[AEM Workflow](/help/forms/using/aem-forms-workflow.md)の設定手順。
...
1. sun.util.calendarがデシリアライゼーションファイアウォール設定に追加されていることを確認します。
   ...
   「デシリアライゼーションファイアウォール設定」に移動し、「パッケージプレフィックスのホワイトリストに登録されたクラス」で、sun.util.calendarを追加します。
