...
1. `https://[server]:[port]/<custom-context-if-any>.`でAEM Formsオーサーインスタンスにアクセスする
...
1. AEM ログインページに資格情報を入力します。
   ...
   ログインしたら、左上隅の&#x200B;**[!UICONTROL Adobe Experience Manager/Forms/Formsとドキュメント]**&#x200B;をタップします。
...
1. 「フォームモデル」タブで、ドロップダウンリストから「**[!UICONTROL フォームテンプレート]**」を選択します。 AEM Forms UI を通じてリポジトリにアップロードされたすべてのフォームテンプレートがリストされ、選択できるようになります。リストからテンプレートを選択します。
   ...
   **図：** *フォームテンプレートの選択*
   ...
   >[!NOTE]
   >
   >アダプティブフォームのフォームテンプレートを変更することもできます。詳しい手順については、「[アダプティブフォームのフォームモデルのプロパティを編集する](#edit-form-model)」を参照してください。
...
1. 「**[!UICONTROL スキーマを選択]**」をタップして、以下のいずれかの操作を行います。
   ...
   * **[!UICONTROL リポジトリを検索]** - このオプションを選択して、リポジトリで使用できるスキーマ定義ファイルのリストから選択します。XML または JSON スキーマファイルをフォームモデルとして選択します。選択したスキーマが参照されてフォームに関連付けられ、他のアダプティブフォームからアクセスして使用できるようになります。
...
テンプレートでは基本的がい造を提供しており、アダプティブフォームの外観（レイアウトやスタイル）を定義します。これには、特定のプロパティやコンテンツ構造を有するフォーマット済みのコンポーネントが含まれます。AEM Forms には、デフォルトでいくつかのアダプティブフォームテンプレートが用意されています。高度なテンプレートを含む完全なテンプレートパッケージを取得するには、AEM Formsアドオンパッケージをインストールする必要があります。 詳しくは、「[AEM Formsアドオンパッケージ](/help/forms/using/installing-configuring-aem-forms-osgi.md)のインストール」を参照してください。
