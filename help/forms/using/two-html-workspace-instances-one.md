...
AEM Forms 管理者はワークスペースをカスタマイズして、2 つの異なる URL を作成し、 2 つのワークスペースを同じサーバー上で使用できるようにします。このカスタマイズ記事では、2つのワークスペースが`https://[server]:[port]/lc/ws`と`https://[server]:[port]:/lc/ws2`でアクセスできると仮定します。
...
1. `GET.jsp`(`/apps/ws2`)で、次のコード変更を行います。 次のコードを
...
1. `/apps/ws2/js`の`registry.js`で、`/apps/ws2/js/runtime/templates`にあるテンプレートを参照するように、テンプレートのパスを変更します。 次のコードを
...
1. `/apps/ws2/js/runtime/models`と`/apps/ws2/js/runtime/views`の`userinfo.js`で、文字列`/lc/content/ws`を`lc/content/ws2`に変更します。

1. `/apps/ws2/js/runtime/services/service.js`で、`getLocalizationData`関数のパスを`/lc/apps/ws2/Locale.html`を指すように変更します。

1. 新しいワークスペースの`pdf.html`を参照するには、`/apps/ws2/js/runtime/views/forms/pdftaskform.js`内の`pdf.html`のパスを変更します。
...
1. `/etc/map/ws`フォルダーをコピーし、`/etc/map`に貼り付けます。 この新しいフォルダーの名前を ws2 に変更します。「すべて保存」をクリックします。
