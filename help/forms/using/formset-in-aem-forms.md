...
「rootElement」という名前はプレースホルダーでしかありません。実際の名前はフォームセットで使用されているフォームから選択されます。rootElementを持つ開始のサブツリーは、フォームセット内のForms内のフィールドとサブフォームのデータを含みます。 RootElement の構造とその子を決定する複数の要素があります。
