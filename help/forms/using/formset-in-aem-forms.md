...
複数の XDP またはフォームテンプレートを、Designer を使用して作成したフォームセットに関連付けることができます。 その後、フォームセットを使用して、最初のフォームにユーザーが入力した値とそのプロファイルに基づいて、XDP を選択的にレンダリングできます。

用途 [AEM Formsユーザーインターフェイス](/help/forms/using/introduction-managing-forms.md) すべてのフォーム、フォームセット、および関連アセットを管理する。
...
1. プロパティの追加ページで、次の詳細を追加して「次へ」をクリックします。
   ...
   * 送信 URL:フォームセットのスタンドアロンレンディションの場合に送信されたデータが投稿される URL を指定します (AEM Forms以外のアプリの使用例 )。 データはマルチパートデータまたはフォームデータとして次のリクエストパラメーターとともにこのエンドポイントに送信されます。
   * dataXML：送信したフォームセットデータの XML 表現を含むパラメータです。フォームセットのすべてのフォームが共通のスキーマを使用している場合、スキーマごとに XML が生成されます。それ以外の場合、XML のルートタグには、フォームの添付ファイルのデータを含むフォームセット内の入力済みのフォームごとに子タグが含まれます。
...
1. フォームを設定のページで、次の操作を実行できます。
   ...
   *fs.valueOf(&lt;form identifier=&quot;&quot;>, &lt;fieldsom expression=&quot;&quot;>) > &lt;value>*
   ...
   例えば、フォームセットに「仕事の経費」と「旅費」という 2 つのフォームがある場合、これらの両方のフォームの Eligibility expression フィールドに Javascript スニペットを追加して、フォームで出費のタイプにユーザーが入力した内容を確認できます。ユーザーが仕事の経費を選択した場合、仕事の経費フォームがエンドユーザーに表示されます。逆に、ユーザーが旅費を選択した場合、エンドユーザーには別のフォームが表示されます。詳細については、「適任性の式」を参照してください。
...
1. フォーム／フォームとドキュメントを選択します。
1. 編集するフォームセットを見つけます。その上にマウスポインターを置いて、「編集」( ![エディティコン](assets/editicon.png)) をクリックします。
...
フォームセットは、共通または異なるスキーマを持つ複数の HTML5 フォームのまとまりです。フォームセットは、XML ファイルを使用したフォームフィールドの事前入力をサポートしています。 XML ファイルをフォームセットに関連付けることで、フォームセットの中のフォームを開いたときに、いくつかのフィールドは事前入力されます。

事前入力XML ファイルは、フォームセットの URL の、dataRef パラメータを使用して指定します。dataRef パラメーターは、フォームセットとマージされるデータ XML ファイルの絶対パスを指定します。
...
>[!NOTE]
>
>XML のルートタグにはどんな名前も指定できますが、フィールドに対応する要素タグにはフィールドと同じ名前を指定しなければなりません。XML の階層は、フォームの階層と同じでなければなりません。つまり、XML には、サブフォームをラッピングするための対応するタグが必要です。

上記の XML のスニペットでは、フォームセットの事前入力 XML が、個々のフォームの事前入力 XML のスニペットのまとまりであることがわかります。異なるフォームの特定のフィールドがお互いに類似したデータ階層 / スキーマを持つ場合、それらのフィールドは同じ値が事前入力されます。この例では、3 つのフォームすべてに、共通のフィールド「field」に同じ値が事前入力されています。 あるフォームから次のフォームにデータを繰り越すシンプルな方法です。フィールドを同じスキーマまたはデータ参照に結合してもデータを繰り越すことができます。フォームセットのデータをフォームのスキーマに基づいて分離することもできます。これを行うには、フォームセット作成中にフォームの「データルート」属性を指定します。デフォルトでは、この値は「/」で、フォームセットのルートタグにマッピングされます。
...
フォームセットでは、XML スキーマは次の構文で定義されます。
...
「rootElement」という名前はプレースホルダーでしかありません。実際の名前はフォームセットで使用されているフォームから選択されます。rootElement で始まるサブツリーには、フォームセット内のForms内のフィールドとサブフォームのデータが含まれます。 RootElement の構造とその子を決定する複数の要素があります。
