...
* [フォームテンプレート](/help/forms/using/creating-adaptive-form.md#p-create-an-adaptive-form-based-on-an-xfa-form-template-p)
   ...
   アダプティブフォームのXFAテンプレートを選択できます。 XFAテンプレートを選択する場合は、前述のように、レコードのドキュメントに関連するXDPファイルを使用できます。
...
* なし
   ...
   フォームモデルを使用せずにアダプティブフォームを作成できます。 レコードのドキュメントはアダプティブフォームに対して自動的に生成されます。

フォームモデルを選択する場合は、「レコードのドキュメントテンプレートの設定」の下にあるオプションを使用して、レコードのドキュメントを設定します。 See [Document of Record Template Configuration](#document-of-record-template-configuration).
...
* データバインディングが処理されます。
* 送信時にレコードのドキュメントから除外するとマークされたフィールドを自動的に非表示にします。 余分な労力が不要になります。
* レコードのドキュメントテンプレートのデザインに要する時間を節約します。
...
**レコードのドキュメントを生成するアダプティブフォーム** 。アダプティブフォームアダプティブフォームを使用します。

**AEM Designerで作成された基本テンプレート** （推奨） XFAテンプレート（XDPファイル）。 基本テンプレートは、レコードのドキュメントテンプレートのスタイル設定とブランド情報を指定するために使用します。
...
**アダプティブフォームから生成されたレコード** XFAテンプレート（XDPファイル）のドキュメント。

See [Document of Record Template Configuration](#document-of-record-template-configuration).

**フォームデータ** ：アダプティブフォームでユーザーが入力した情報。 レコードのドキュメントを生成するために、レコードのドキュメントテンプレートとマージします。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> </td> 
   <td>false</td> 
   <td>レコードのドキュメントテンプレートでは使用できません。 添付ファイルを使用したレコードのドキュメントでのみ使用できます。</td> 
  </tr> 
 </tbody> 
</table>
...
| アダプティブフォームコンポーネント | 対応する XFA スキンコンポーネント | メモ |
|---|---|---|
| 画像 | 画像 | TextDrawおよびImageコンポーネントは、連結されているか連結されていないかにかかわらず、XSDベースのアダプティブフォームのレコードのドキュメントに常に表示されます。ただし、レコードのドキュメントを使用して除外される場合は除きます。 |
| テキスト | テキスト |
...
基本テンプレートは、レコードのドキュメントのヘッダー、フッター、スタイル、および外観を定義するために使用されます。ヘッダーとフッターには、会社のロゴや著作権テキストなどの情報を含めることができます。基本テンプレートの最初のマスターページがコピーされ、レコードのドキュメントのマスターページとして使用されます。このページには、レコードのドキュメント内のすべてのページに表示するヘッダー、フッター、ページ番号などの情報が含まれます。 基本テンプレートの規則に準拠しない基本テンプレートを使用している場合、基本テンプレートの最初のマスターページは、レコードのドキュメントテンプレートで引き続き使用されます。 基本テンプレートを規則通りに設計し、レコードのドキュメント自動生成のためにそれを使用することを強くお勧めします。
...
* これらのフィールドのプロパティは、レコードのドキュメント内のフィールドに適用されます。これらのフィールドは、命名規則に従う `AF_<name of field in all caps>_XFO` 必要があります。 For example, the field name for check box should be `AF_CHECKBOX_XFO`.
...
顧客が送信したフォームの印刷用フレンドリコピーをダウンロードできるように、フォームのレコードのドキュメントテンプレートを設定します。 XDPファイルは、レコードテンプレートのドキュメントとして機能します。 顧客がダウンロードしたレコードのドキュメントは、XDPファイルで指定されたレイアウトに従って形式設定されます。
...
1. In the Properties window, tap **Form Model**.
   ...
   >[!NOTE]
   >
   >「フォームモデル」のタブで、必ず&#x200B;**「フォームの選択」**&#x200B;ドロップダウンから&#x200B;**「 スキーマ」**&#x200B;または&#x200B;**「なし」**&#x200B;を選択します。**[!UICONTROL レコードのドキュメントは、XFAベースのフォームや、フォームテンプレートをフォームモデルとして持つアダプティブフォームではサポートされていません。]**
...
1. 「フォームモデル」タブの「レコードのドキュメントのテンプレート設定」セクションで、次のいずれかのオプションを選択します。
   ...
   **レコードのドキュメントを生成する基本テンプレートとしてフォームテンプレートを選択** XDPファイルをレコードのドキュメントのスタイルと外観を定義する基本テンプレートとして使用する場合は、このオプションを選択します。 このオプションを選択すると、AEM Forms リポジトリで利用可能なすべての XDP ファイルが表示されます。必要なファイルを選択します。
...
1. ドータ ![ブをタップ](assets/dortab.png)。 「レコードのドキュメント」タブが表示されます。
1. レコードのドキュメントをレンダリングする、デフォルトのテンプレートまたはカスタムのテンプレートのいずれかを選択します。デフォルトのテンプレートを選択すると、レコードのドキュメントのサムネイルが「テンプレート」ドロップダウンの下に表示されます。
   ...
   ![ブランディングテンプレート](assets/brandingtemplate.png)
...
1. デフォルトのテンプレートとカスタムのテンプレートのどちらを選択するかにより異なりますが、以下のプロパティの一部または全部が「レコードのドキュメント」タブに表示されます。これらを正しく設定します。
   ...
   * **Logo Image**:アダプティブフォームのロゴ画像の使用、DAMからのロゴ画像の選択、またはコンピューターからのロゴ画像のアップロードを選択できます。
   * **フォームタイトル**
   * **ヘッダーテキスト**
   * **免責事項ラベル**
   * **免責事項**
   ...
   * **Font Family**:レコードPDFのドキュメント内のテキストのフォントファミリー
...
コンポーネントのレコード設定のドキュメントは、そのプロパティの下で使用できます。 To access the properties a component, select the component and click ![cmppr](assets/cmppr.png) in the overlay. プロパティがサイドバーに表示され、次の設定を確認できます。
...
* **** パネルを表として表示：パネルに含まれるフィールドが6個未満の場合、このプロパティを設定すると、パネルがレコードのドキュメント内のテーブルとして表示されます。 この設定は、パネルにのみ適用されます。
...
* レコードのドキュメントテンプレートはリッチテキストをサポートしていません。 そのため、スタティックアダプティブフォームまたはエンドユーザー入力情報に含まれるリッチテキストは、レコードのドキュメントではプレーンテキストとして表示されます。
