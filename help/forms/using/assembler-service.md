...
![](assets/as_document_assembly.png) 複数のPDFドキュメントからの単純なPDFドキュメントのアセンブリ&#x200B;****&#x200B;図：複数のPDF *ドキュメントからの単純なPDFドキュメントのアセンブリ*
...
![](do-not-localize/as_batesnumber.png) ドキュメントのヘッダー図に一意の識別子を含む&#x200B;**** PDFドキュメント。ドキ *ュメントのヘッダーに一意の識別子が含まれるPDFドキュメント*
...
![](assets/as_assembler_xdpassembly.png) 複数のXDPドキュメントからの単純なXDPドキュメントのアセンブリ&#x200B;****&#x200B;図：複数 *のXDPドキュメントからの単純なXDPドキュメントのアセンブリ*
...
![](assets/as_assembler_fragment_assy_assembled.png) XFAフォーム図へのフォームフラグメントの&#x200B;****&#x200B;挿入：XFAフォ *ームへのフォームフラグメントの挿入*
...
![](assets/as_intro_page_extraction.png) ソースドキュメント図から特定のページを抽出&#x200B;****&#x200B;する：ソー *スドキュメントからの特定のページの抽出*
...
![](assets/as_intro_pdfsfrombookmarks.png) しおりに基づくソースドキュメントの複数のドキュメントへの分割図&#x200B;****:しおり *に基づくソースドキュメントの複数のドキュメントへの分割*
...
**Output サービス**：動的な XML フォームを、非インタラクティブフォームを含む PDF ドキュメントに変換（フォームを統合）します。Assemblerサービスは、Outputサービスを呼び出さずに、静的XMLフォームとAcrobatフォームを統合します。
...
DDX と Assembler サービスを使用して他の LiveCycle サービスを呼び出すことによって、プロセスダイアグラムが単純化され、ワークフローをカスタマイズする手間を減らすことができます。(関連項目, [AEM Document Services をプログラムとして使用する](https://helpx.adobe.com/experience-manager/6-4/forms/using/aem-document-services-programmatically.html))
