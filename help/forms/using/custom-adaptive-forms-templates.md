...
* 基本：左側のタブのレイアウトを使用してマルチタブのアダプティブフォームを作成し、任意の順序でタブにアクセスできます。
...
1. **（既にコンテンツページがある場合のみ）** webサイト用の既存のコンポーネントがある場合は、次の手順(a ～ d) `contentpage`を実行します。 自分の Web サイトに `contentpage` コンポーネントが存在しない場合は、`resourceSuperType` プロパティを OOTB の基本ページにポイントしたままにすることができます。
