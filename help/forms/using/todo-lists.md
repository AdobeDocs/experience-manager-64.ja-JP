...
1. 次のいずれかの手順を実行します。
   ...
   * フォームにボタンが含まれ、AEM Forms Workspaceの「完了」ボタンを使用できない場合は、フォーム上で適切なボタンをクリックして、プロセス内の次の手順に進みます。
...
まず、電子メール通知を受信するようにAEM Forms Workspaceの環境設定を行います。 AEM Forms Workspaceでは、TODOリスト内のタスクや、自分が属するグループのTODOリストに対して、電子メール通知を送信できます。 電子メール通知メッセージを送信するタイミングやその受信者は、管理者が決定します。
...
>[!NOTE]
>
>ユーザーがブラウザーでAEM Forms Workspaceにログインせず、TODOタスクへのリンクを開いた場合、TODOリンクに直接アクセスすると、リンクは開かず、例外が表示されます。 電子メール内のリンクをクリックする前に、AEM Forms Workspaceにログインします。
