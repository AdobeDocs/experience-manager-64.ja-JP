...
クラウド設定とフレームワークを探すには、**ツール**/**デプロイメント**/**Cloud**&#x200B;を介して&#x200B;**Cloud Services**&#x200B;に移動します。 （[http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html)）。\
Adobe Targetの下の「**設定を表示**」をクリックまたはタップします。
...
1. 「フレームワークを作成」ダイアログで、**タイトル**&#x200B;を指定し、**Adobe Targetフレームワーク**&#x200B;を選択して、「**作成**」をクリックします。
   ...
   フレームワークページが表示されます。サイドキックは、マッピング可能な[クライアントコンテキスト](/help/sites-administering/client-context.md)または[ContextHub](/help/sites-administering/contexthub-config.md)からの情報を表すコンポーネントを提供します。
