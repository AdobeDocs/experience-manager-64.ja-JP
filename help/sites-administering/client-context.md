...
JSONP 標準は JSON を補完し、同一生成元ポリシーを回避します（Web アプリケーションが別のドメインにあるサーバーと通信できないようにします）。JSONオブジェクトを他の接触チャネルから読み込む（同じドメインポリシーで許可されている例外）ために、JSONオブジェクトを関数呼び出しでラップすることで構成されます。
...
ClientContext を編集して、特定のプロパティの値を設定（またはリセット）できます。This allows you to test specific scenarios (particularly useful for [segmentation](/help/sites-administering/campaign-segmentation.md) and [campaigns](/help/sites-authoring/personalization.md)).
