...
JSONP 標準は JSON を補完し、同一生成元ポリシーを回避します（Web アプリケーションが別のドメインにあるサーバーと通信できないようにします）。JSONオブジェクトを他のドメイン（同じオリジンポリシーで許可される例外）から読み込めるように、関数呼び出しでJSONオブジェクトをラップすることで構成されます。
