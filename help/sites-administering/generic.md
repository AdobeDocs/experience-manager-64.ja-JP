...
>
>つまり、デフォルトでは [Dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html) によって商品画像がブロックされるので、必要に応じて設定してください。
...
1. 「**Change **」をタップまたはクリックすると、ダイアログが開き、アセットを選択できます。
1. 追加するアセットを選択します。
...
1. 形状をクリックします。
1. Clicking the browse icon opens the [Asset selector](/help/assets/asset-selector.md#using-the-asset-selector).
...
>
>AEMの汎用注文管理は基本的に保たれています。ウィザードで使用できるフィールドは、スキャフォールドに依存します。\
