...
>[!NOTE]
>
>事前に設定されたレポートはパフォーマンスを集中的に消費するわけではありませんが、実稼働環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Webサイトにあまりアクティビティがない時間帯に毎日のスナップショットを実行します。 これは、 `Daily snapshots (repconf.hourofday)` Day CQレポート設定の **パラメーターを使用して定義できます**。 この設定方法の詳細は、 [OSGI設定](/help/sites-deploying/configuring-osgi.md) (OSGI Configuration)を参照してください。
