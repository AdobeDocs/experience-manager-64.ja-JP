...
* **間隔**
   ...
   * 間隔を`Day`に設定した場合、各スナップショットはグラフに1つの値として表示されます。
   * 間隔を`Month`に設定した場合、2月のすべてのスナップショットは1つの値に集計されます（グラフには1つの「ドット」として表示されます）。
...
* **編集**&#x200B;ダイアログを開き、「**データスナップショットなし**」を選択して、必要な時間までコレクションを終了します。
   ...
   **[** 編集のみ]：スナップショットの取り込みのオン/オフを切り替えます。スナップショットの作成が再び有効になった場合は、最後に完了したレポートの状態が使用され、さらにスナップショットを作成できます。

>[!NOTE]
>
>スナップショットは`/var/reports/...`に保存されます。パスの残りの部分は、レポートの終了時に作成された各レポートのパスとIDを反映しています。
>
>
...
* **レポート処理**
   ...
   * **データを手動で更新**
      ...
      このオプションを選択すると、「**[データを読み込み](#load-data)**」ボタンが表示されます（レポートの「**編集**」の横）。 **データを読み込む** と、データが読み込まれ、表示されているレポートデータが更新されます。
...
ページアクティビティレポートは、監査ログからすべての情報を取得します。デフォルトでは、ルートパスは`/var/audit/com.day.cq.wcm.core.page`の監査ログに設定されます。
...
* **型**
   ...
   `String`、`Number`、`Integer`、`Date`からフィールドタイプを選択します。

* **デフォルト集計**

   これは、少なくとも1つの列がグループ化されたレポートで列のグループが解除された場合にデフォルトで使用される集計を定義します。 `Count`、`Minimum`、`Average`、`Maximum`、`Sum`から必要な集計を選択します。

   例えば、*`String`フィールドのカウント*&#x200B;は、集計状態の列に個別の`String`値の数が表示されることを意味します。
...
>[!CAUTION]
>
>これらのレポートへのアクセスはセキュリティ上の問題となる場合があります。したがって、`/etc/reports`を外部訪問者が使用できないようにディスパッチャーを設定することをお勧めします。 詳しくは、[セキュリティチェックリスト](security-checklist.md)を参照してください。
