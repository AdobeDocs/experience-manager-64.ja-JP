...
>[!NOTE]
>
>これらのレポートは、クラシック UI でのみ使用できます。 最新の UI でのシステム監視とレポートについては、 [操作ダッシュボード。](/help/sites-administering/operations-dashboard.md)
...
* **間隔**
   ...
   レポートの規模と集計には、月、週、日、時間を選択できます。

   例えば、2011 年 2 月に日別のスナップショットを利用できる場合は、次のようになります。

   * 間隔が `Day`の場合、各スナップショットは、グラフに単一の値として表示されます。
   * 間隔が `Month`、2 月のすべてのスナップショットは 1 つの値に集計されます（グラフに 1 つの「ドット」として表示されます）。
...
* 用途 **完了…** コレクションを再初期化する場合に再度使用します。

   **完了** レポートの構造（レポートに割り当てられ、グループ化、並べ替え、フィルター処理された列など） とがスナップショットの作成を開始します。

* を開きます。 **編集** 選択するダイアログ **データスナップショットがありません** 必要に応じてコレクションを終了します。

   **編集** スナップショットの取得のオン/オフを切り替えるだけです。 スナップショットの取得が再度オンになった場合は、さらにスナップショットを取得するために最後に完了した時点のレポートの状態が使用されます。

>[!NOTE]
>
>スナップショットは、以下の場所に保存されます。 `/var/reports/...` パスの残りの部分は、各レポートのパスと、レポートの完了時に作成された ID を反映しています。
>
>
...
>[!NOTE]
>
>事前設定済みのレポートはパフォーマンスを集中的に消費しませんが、実稼動環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Web サイト上でのアクティビティが多くない時間帯に、これらの毎日のスナップショットを実行します。これは、 `Daily snapshots (repconf.hourofday)` パラメータ **Day CQ Reporting Configuration**;参照 [OSGi 設定](/help/sites-deploying/configuring-osgi.md) を参照してください。
...
* **レポート処理**
   ...
   * **データを手動で更新**
      ...
      これを選択すると、 **[データを読み込み](#load-data)** ボタンが ( **編集** を参照 )。 **データを読み込み** はデータを読み込み、表示されているレポートデータを更新します。
...
* どのコンポーネントが、どこで使用されるか。

   例えば、テスト時に役立ちます。

* 特定のコンポーネントのインスタンスの分散方法。

   これは、特定のページ (「重いページ」など ) で、パフォーマンスの問題が発生しています。
...
![reportcomponent](assets/reportcomponent.png) ![reportcomponentall](assets/reportcompentall.png)
...
ページアクティビティレポートは、監査ログからすべての情報を取得します。デフォルトでは、ルートパスは次の監査ログに設定されます。 `/var/audit/com.day.cq.wcm.core.page`.
...
* **プロパティ**
   ...
   リポジトリに保存されるプロパティ名。通常はユーザーのプロファイル内にあります。
...
* **タイプ**
   ...
   次の中からフィールドタイプを選択します。 `String`, `Number`, `Integer`, `Date`.

* **デフォルトの集計**

   これは、少なくとも 1 つのグループ化された列を含むレポートで列のグループ化が解除された場合にデフォルトで使用される集計を定義します。 必要な集計の選択元 `Count`, `Minimum`, `Average`, `Maximum`, `Sum`.

   例： *カウント* の `String` フィールドは、ユニークな `String` の値が集計状態で列に表示されます。

「**拡張**」タブでは、使用可能な集計およびフィルターも定義できます。

![reportusrgenericcolmextensed](assets/reportusrgenericcolmextented.png)
...
>[!CAUTION]
>
>これらのレポートへのアクセスは、セキュリティ上の問題となる場合があります。したがって、次のように Dispatcher を設定することをお勧めします。 `/etc/reports` は、外部の訪問者は使用できません。 詳しくは、[セキュリティチェックリスト](security-checklist.md)を参照してください。
