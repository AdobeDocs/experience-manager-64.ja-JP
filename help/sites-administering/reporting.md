...
>[!NOTE]
>
>事前に構成されたレポートはパフォーマンスを集中的に消費するわけではありませんが、実稼働環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Webサイト上での活動が少ない時間帯に毎日のスナップショットを実行します。これは、 `Daily snapshots (repconf.hourofday)` Day CQ Reporting Configurationのパラメータ **ーを使**&#x200B;用して定義できます。この設定方 [法の詳細は](/help/sites-deploying/configuring-osgi.md) 、「OSGI設定」を参照してください。
