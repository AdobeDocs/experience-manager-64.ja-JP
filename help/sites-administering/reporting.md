...
>[!NOTE]
>
>これらのレポートは、クラシックUIでのみ使用できます。 最新のUIでのシステム監視とレポートについては、[操作ダッシュボード](/help/sites-administering/operations-dashboard.md)を参照してください。
...
1. **終了…** スナップショットの収集を開始する定義。

   左上の赤/緑のスライダ・ボタンは、スナップショットを収集するタイミングを示します。
...
* **間隔**
   ...
   レポートのスケールと集計に、月、週、日、時間を選択できます。

   例えば、2011年2月に日次スナップショットを使用できる場合は、次の手順に従います。

   * 間隔を`Day`に設定すると、各スナップショットが1つの値としてグラフに表示されます。
   * 間隔を`Month`に設定した場合、2月のすべてのスナップショットが1つの値に集計されます（グラフに1つの「ドット」として表示されます）。
...
* **終了…を使用します。**&#x200B;を再度呼び出して、コレクションを再初期化します。

   **** を終了すると、レポートの構造が「凍結」されます（レポートに割り当てられている列や、グループ化、並べ替え、フィルター処理されている列など）。スナップショットの作成を開始します。

* **編集**&#x200B;ダイアログを開き、「**データスナップショットなし**」を選択して、必要な時間までコレクションを終了します。

   **** Editonlyは、スナップショットの取得のオン/オフを切り替えます。スナップショットの取得が再度有効になった場合は、さらにスナップショットを取得するために最後に完了した時点のレポートの状態が使用されます。

>[!NOTE]
>
>スナップショットは`/var/reports/...`に保存されます。パスの残りの部分は、各レポートのパスと、レポートの終了時に作成されたIDを反映します。
>
>
...
>[!NOTE]
>
>事前設定されたレポートはパフォーマンスを集中的に消費するわけではありませんが、実稼動環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Webサイト上でのアクティビティが少ない時間帯に、これらの日別のスナップショットを実行します。これは、**Day CQ Reporting Configuration**&#x200B;の`Daily snapshots (repconf.hourofday)`パラメーターを使用して定義できます。この設定方法について詳しくは、[OSGI設定](/help/sites-deploying/configuring-osgi.md)を参照してください。
...
* **レポート処理**
   ...
   * **データを手動で更新**
      ...
      これを選択すると、レポート設定のいずれかの側面が変更された場合に、レポートデータを手動で更新する必要があります。 また、設定の任意の側面を変更すると、レポートテーブルは空白になります。

      これを選択すると、「**[データを読み込み](#load-data)**」ボタンが表示されます（レポートの「**編集**」の横）。 **データを** 読み込むと、データが読み込まれ、表示されているレポートデータが更新されます。
...
1. レポート設定を変更すると、すぐにレポートデータのテーブルが空白になります。

   例えば、列の並べ替えメカニズムを変更すると、データは表示されません。
...
* 特定のコンポーネントのインスタンスの分散方法。
   ...
   これは、特定のページ(「重いページ」など)で、パフォーマンスの問題が発生している問題を修正しました。
...
を使用すると、特定の期間内で最もコストの高いリクエストを特定できます。
...
ページアクティビティレポートは、監査ログからすべての情報を取得します。デフォルトでは、ルートパスは`/var/audit/com.day.cq.wcm.core.page`にある監査ログに設定されます。
...
![reportusergenericcolm](assets/reportusrgenericcolm.png)
...
* **プロパティ**
   ...
   リポジトリに保存されるプロパティ名（通常はユーザーのプロファイル内）。
...
* **型**
   ...
   フィールドタイプを`String`、`Number`、`Integer`、`Date`から選択します。

* **デフォルトの集計**

   これは、少なくとも1つの列がグループ化されたレポートで列のグループ化が解除された場合にデフォルトで使用される集計を定義します。 必要な集計を`Count`、`Minimum`、`Average`、`Maximum`、`Sum`から選択します。

   例えば、`String`フィールドの&#x200B;*カウント*&#x200B;は、集計状態で列に個別の`String`値の数が表示されることを意味します。
...
![workflow](assets/reportworkflow.png)
...
>[!CAUTION]
>
>これらのレポートへのアクセスは、セキュリティ上の問題である可能性があります。そのため、外部の訪問者が`/etc/reports`を使用できないようにDispatcherを設定することをお勧めします。 詳しくは、[セキュリティチェックリスト](security-checklist.md)を参照してください。
