...
タグ付けコンソールは、タグと分類の作成および管理に使用します。一つの目的は、基本的に同じことに関連する類似のタグを多く持つのを避けることです。例えば、ページやページ、靴や靴など。
...
![managing_tags_usingtagasadministrationconsole](assets/managing_tags_usingthetagasministrationconsole.png)
...
* **Title***（必須） *名前空間の表示タイトル。

* **Name***（オプション） *名前空間の名前。 指定しない場合、有効なノード名が「タイトル」から作成されます。See [TagID](/help/sites-developing/framework.md#tagid).
...
最初に選択したときに、名前空間にタグが含まれていない場合は右にプロパティが表示され、含まれている場合は子タグが表示されます。選択した各タグに子タグがない場合は、そのタグに含まれるタグまたはそのプロパティが表示されます。
...
When a namespace or other tag is selected, selecting the **`View Properties`** icon results in the display of information as to the `name`, time of last edit, and number of references. 発行された場合は、前回発行された時刻と発行者のIDが表示されます。 この情報は、タグ列の左側の列に表示されます。
...
* **「名前を最初**&#x200B;に変更」は、タグの現在 `name`の名前を表示します。 A new `name`may be entered.
...
When a namespace or other tag is selected, selecting the **`Publish`**icon to activate the tag in the publish environment. ページコンテンツと同様に、選択したタグのみが発行されます。コンテナタグであるかどうかは関係ありません。
...
Tag permissions are [&#39;secure (by default)&#39;](/help/sites-administering/production-ready.md); a best practice for the publish environment that requires read permission to be explicitly allowed for tags. 基本的に、これは、作成者に権限を設定した後でタグ名前空間のパッケージを作成し、すべての発行インスタンスにパッケージをインストールすることで行います。
...
* すべてのパブリッシュインスタンスが同じ権限となるようにします
   ...
   * 1 つのアプローチは、オーサー環境で名前空間の[パッケージを作成](/help/sites-administering/package-manager.md#package-manager)することです
      ...
      * タブ `Advanced` で、選択 `AC Handling` 用 `Overwrite`
   * パッケージをレプリケートします
...
タグ付けコンソールでは、ページの言語設定の代わりに、ユーザーの言語設定が使用されます。タグ管理コンソールで、「動物」タグに対して、ユーザプロパティで言語をフランス語に設定したユーザに対して「アニモー」が表示されます。
...
* [開発者向けタグ付け](/help/sites-developing/tags.md)
   ...
   タグ付けフレームワークに関する情報と、カスタムアプリケーションでタグを拡張および含める方法に関する情報です。
