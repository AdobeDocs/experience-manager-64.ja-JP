...
>[!NOTE]
>
>この記事での重複を避けるために、それぞれのrtePluginsノードを&lt;*rtePlugins-node*&#x200B;と呼びます。
...
>[!NOTE]
インストールされていない言語のチェックを試みると、メッセージ`Spell checking failed`が表示されます。 標準辞書は、readme ファイルと一緒に `/libs/cq/spellchecker/dictionaries` にあります。これらのファイルを修正しないでください。
