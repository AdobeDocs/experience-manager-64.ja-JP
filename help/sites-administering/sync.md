...
* 見つける `Apache Sling Distribution Packaging - Vault Package Builder Factor`

   * `Builder name: socialpubsync-vlt`

* 編集アイコンを選択します。
* 2を追加しま `Package Filters` す。
...
このリストには、診断が実行される各パブリッシュインスタンスの URL が記載されています。The url param `syncUser` is appended to the diagnostics URL with its value set to the *authorized sync user* created in [Step 2](/help/sites-administering/sync.md#createauthuser).
