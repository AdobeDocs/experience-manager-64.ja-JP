...
**ユーザーアカウント** CRXは、ユーザーアカウントに保持されている詳細に従って、ユーザーを（そのユーザーまたは他のアプリケーションによって）識別し、検証することで、アクセスを認証します。
...
**Access Rights** CRXは、Access Rightsを使用して、リポジトリの特定の領域へのアクセスを制御します。
...
* サブジ **ェクトは** 、リクエストのソースを表すために使用されます。

   これは、その要求に適用されるアクセス権を統合するために使用されます。 これらは次の場所から取得されます。
...
* `aUser` に対する書き込み権限が与えられていませ `grandChildNode`ん。
...
**偽装者** ：ユーザーが別のユーザーの代わりに作業できる、偽装機能を使用します。
...
1. 適切なアカウント用の&#x200B;**ユーザー管理**&#x200B;ダイアログを開きます。
1. そのアカウントを偽装することを許可するアカウントを指定します。

   「参照」を使用できます。をクリックして、既存のアカウントを選択します。
...
**メンバー** ：現在のグループのメンバーであるすべてのアカウント（ユーザーまたはグループ）をリストします。
...
* **ローカルアクセス制御ポリ**&#x200B;シーこれらは、ユーザーが適用したアクセス制御ポリシーです。その後、更新、並べ替えまたは削除できます。
