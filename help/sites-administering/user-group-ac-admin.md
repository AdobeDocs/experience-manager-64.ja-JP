...
description: AEMでのユーザー、グループ、アクセス権の管理について説明します。
feature: Security
...
>[!NOTE]
>
>CRX 実装 [JSR-283 で定義されるアクセス制御](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html).
>
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><i>グループを使用する</i></td> 
   <td><p>アクセス権をユーザーごとに割り当てないようにします。 これには、いくつかの理由があります。</p> 
    <ul> 
     <li>グループよりも多くのユーザーがいるので、グループを使用すると構造が簡単になります。</li> 
     <li>グループは、すべてのアカウントの概要を示すのに役立ちます。</li> 
     <li>グループを使用すると、継承が簡単になります。</li> 
     <li>ユーザーが来たり来たりします。 グループは長期的なものです。</li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td><i>ポジティブに</i></td> 
   <td><p>常に Allow ステートメントを使用して、グループプリンシパルのアクセス権を指定します（可能な場合）。 「拒否」ステートメントの使用は避けてください。</p> <p>グループプリンシパルは、階層内でも、単一のアクセス制御リスト内でも順番に評価されます。</p> </td> 
  </tr> 
  <tr> 
   <td><i>シンプルに</i></td> 
   <td><p>新しいインストールの設定時にある程度時間をかけて検討すると、より良い設定となります。</p> <p>明確な構造を適用すると、継続的なメンテナンスと管理が簡単になり、現在の同僚や将来の後継者が実装されている内容を容易に理解できるようになります。</p> </td> 
  </tr> 
  <tr> 
   <td><i>テスト</i></td> 
  ...
  <tr> 
   ...
   <td>セキュリティ上の問題を防ぐため、インストール直後に必ずデフォルトのユーザーとグループを更新してください。</td> 
  </tr> 
 </tbody> 
</table>
...
1. 変更を加え、そのエントリの保存（緑のチェックマーク）をクリックします。
1. クリック **閉じる** 終了するか、または **リスト…** をクリックして、すべてのユーザーアカウントのリストに戻ります。
...
1. を開きます。 **ユーザー管理** 適切なアカウントのダイアログ。
1. を定義 **プロパティ** 名前。
1. を選択します。 **タイプ** 」を選択します。
1. 次を定義： **値**.
1. 新しいプロパティの [ 保存 ] （緑のクリック記号）をクリックします。
...
1. 新しいプロパティの [ 保存 ] （緑のチェックマーク）をクリックします。
...
1. 編集用に開く必要のあるアカウントをクリックします。
1. 変更を加え、そのエントリの保存（緑のチェックマーク）をクリックします。
1. クリック **閉じる** 終了するか、または **リスト…** をクリックして、すべてのグループアカウントのリストに戻ります。
...
`jcr:namespaceManagement` 権限。ノードではなく、リポジトリにのみ関連します。
...
アクセス制御エントリを追加する場合は、以下に示す権限を選択できます（詳しくは、[セキュリティ API](https://docs.adobe.com/docs/jp/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/security/Privilege.html) に関するページを参照）。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>mixin ノードタイプを追加および削除し、ノードのプライマリノードタイプを変更します。 また、Node.addNode および XML インポートメソッドの呼び出しも含まれ、mixin または新しいノードのプライマリ型が明示的に指定されます。</td> 
  </tr> 
  <tr> 
...
1. アクセス制御ポリシーを定義し、 **OK** 保存します。 新しいポリシーでは、次のことが行われます。
