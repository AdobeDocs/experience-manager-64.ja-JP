...
HMAC キーがのバイナリプロパティとして存在する `/etc/key` リポジトリ内に保存されます。 その横にある **View** リンクをクリックすることで別個にダウンロードできます。
...
1. ブラウザーで次の場所を指定します。 `https://serveraddress:port/system/console/configMgr`
