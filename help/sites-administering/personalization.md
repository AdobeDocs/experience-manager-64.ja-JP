...
* 個人とグループの両方のプロファイルを持っている。 このプロファイルには職務内容、場所、趣味などの情報が含まれます。この情報は、表示するコンテンツのパーソナライゼーションをおこなう際に使用できます。
* 行動を起こす。 このアクションを分析して行動ルールと照合することで、表示するコンテンツが調整されます。

### コンテンツ {#content}

* ユーザーが表示したい内容です。 ユーザーの関心を引き、ユーザーが実行したいタスクに役立つコンテンツにすることが望まれます。
* 分類できるので、事前定義されたルールに従ってユーザーが使用できます。動的である必要があります。言い換えれば、内容です。
* ある意味で、ユーザーに依存する必要があります。すべてのユーザーが同じコンテンツを表示する場合、パーソナライゼーションは冗長になります。

### ルール {#rules}

* パーソナライゼーションの実際の実行方法（ユーザーに表示されるコンテンツとタイミング）を定義します。
...
* 内部ネットワーク内で既に定義されている、ユーザーの場所、部門、役割に基づいて、コンテンツを提供できます。
...
### 販売・流通ウェブサイト {#sales-site}

* Amazon などの販売流通 Web サイトでは、ユーザープロファイル、ユーザーの購入履歴や閲覧履歴を組み合わせて、ユーザーが次に関心を寄せる可能性がある商品を提案できます。

### Web サイトを検索 {#search-site}

* 主要な検索エンジン Web サイトの多くには、非常に強力な分析ツールが備わっており、ユーザーの行動、ユーザーが使用した検索語句、ユーザーが訪問した Web サイトが記録されます。これは、提供されるコンテンツをカスタマイズするために使用されます。特に、広告の表示に関して使用されます。
...
* キャッシュとは、パフォーマンスと正確性の面で、Web サイトがパーソナライズされたコンテンツをどの程度の速さで配信し、常に最新の状態に保つかを示す要素です。
* パーソナライゼーションを設定する場合、キャッシュは重要な考慮事項です。時間をかけて検討し、適切な実装がおこなわれるようにしてください。

>[!TIP]
>
>パフォーマンスに対するパーソナライゼーションの影響と関連するキャッシュに関するトピックについては、ドキュメントで詳しく説明します [パフォーマンスの最適化。](/help/sites-deploying/configuring-performance.md)
...
* したがって、ルールは、ユーザーの要件を前景に示して、十分に考慮する必要があります。 この作業は手間がかかりますが、軽視すべきではありません。パーソナライゼーションの実装では多くの場合、テクニカルな作業よりビジネスルールを定義することが優先されます。

#### 使用するタイミング {#when-to-use}

* Web 上の多くの機能と同じように、パーソナライゼーションは慎重に実行する必要があります。この機能は本当にユーザーの役に立つのか、あるいは、より少ない労力で目的のページへ到達できる方法が他にないか、常に最初に考慮するべきです。パーソナライゼーションは、実際にメリットをもたらさないので、1 回だけ（どのように機能するかを確認するために）設定した機能であるリスクを実行できます。
* パーソナライゼーションは、コンテンツが動的な場合にのみ意味を持ち、何らかの方法でユーザーに依存します。 すべてのユーザーに同じコンテンツが表示される場合、パーソナライゼーションは不必要です。
...
ブログ、フォーラム、カレンダーなど、結果としてコミュニティのコンテンツが作成されるコミュニティの機能は、一般的にユーザー生成コンテンツ（UGC）と呼ばれます。複数のAEMインスタンス ( [パブリッシュファーム](/help/communities/topologies.md)) では、すべてのインスタンス間で UGC を同期する方法が主な問題の 1 つになりました。

を使用 [AEM Communities 6.1](/help/communities/overview.md) 拡張機能を使用すると、この問題は [UGC 用の共通ストア](/help/communities/working-with-srp.md). パーソナライゼーションに関しては、コミュニティには以下が含まれます。 [ソーシャルログイン](/help/communities/social-login.md) ：サイト訪問者がFacebookおよびTwitterを使用してログインするためのオプションを提供する機能。

Communities の拡張機能がない場合、UGC の整合性の問題に対処する方法は次のとおりです。

* 必要に応じて、複数のパブリッシュインスタンスを同期します
* パブリッシュインスタンスからオーサー環境に UGC を送信し、そこからページコンテンツのパブリッシュと同様の方法で UGC をパブリッシュできます。
