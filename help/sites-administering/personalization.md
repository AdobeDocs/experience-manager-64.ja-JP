...
source-git-commit: e24f308c72c8ad4a6ac93efa74c8e63a4ded6700
workflow-type: tm+mt
...
* プロファイル（個人とグループの両方）がある。 このプロファイルには職務内容、場所、趣味などの情報が含まれます。この情報は、表示するコンテンツのパーソナライゼーションをおこなう際に使用できます。
* 行動を起こす。 このアクションを分析して行動ルールと照合することで、表示するコンテンツが調整されます。

### コンテンツ {#content}

* ユーザーが表示したいもの。 ユーザーの関心を引き、ユーザーが実行したいタスクに役立つコンテンツにすることが望まれます。
* 分類でき、事前定義されたルールに従ってユーザーが利用できるようにします。動的である必要があります。言い換えれば、内容は
* ある意味では、ユーザーに依存する必要があります。すべてのユーザーが同じコンテンツを表示する場合は、パーソナライゼーションは不要です。
...
#### 特定、制限、ターゲットユーザーグループ — エクストラネット{#extranets}
...
>[!TIP]
>
>パフォーマンスと関連するキャッシュに対するパーソナライゼーションの影響については、ドキュメント[パフォーマンスの最適化](/help/sites-deploying/configuring-performance.md)で詳しく説明しています。
...
* 必要に応じて複数の発行インスタンスを同期
* 発行インスタンスから発行環境にUGCを送信し、そこからページコンテンツの発行と同様の方法で発行できるようにします
