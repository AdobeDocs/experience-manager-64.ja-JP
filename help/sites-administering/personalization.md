...
* は、ある意味で、ユーザーに依存する必要があります。すべてのユーザーが同じコンテンツを表示する場合、パーソナライゼーションは冗長になります。

**ルール**

* パーソナライゼーションの実際の動作（ユーザーが表示できるコンテンツとタイミング）を定義します。
...
* コンテンツは、内部ネットワーク内で既に定義されているユーザーの場所、部門、役割に基づいて提供できます。
...
パーソナライゼーションを使用する場合は、以下の点を考慮してください。
...
* キャッシュは、パフォーマンスと正確性の点で、Webサイトがパーソナライズされたコンテンツをどれだけ迅速に配信し、常に最新の状態に保つかという点で、ユーザーに表示される側面です。
...
* したがって、ルールは、ユーザーの要件を前景に考慮して十分に検討する必要があります。 この作業は手間がかかりますが、軽視すべきではありません。パーソナライゼーションの実装では多くの場合、テクニカルな作業よりビジネスルールを定義することが優先されます。

**用途**

* Web 上の多くの機能と同じように、パーソナライゼーションは慎重に実行する必要があります。この機能は本当にユーザーの役に立つのか、あるいは、より少ない労力で目的のページへ到達できる方法が他にないか、常に最初に考慮するべきです。パーソナライゼーションは、ユーザが一度設定した機能（機能を確認するため）と、実際に利点を生み出さないために1回だけの機能となるリスクを伴います。
* パーソナライゼーションは、コンテンツが動的な場合にのみ意味を持ち、何らかの形でユーザーに依存します。 すべてのユーザーに同じコンテンツが表示される場合、パーソナライゼーションは不必要です。
