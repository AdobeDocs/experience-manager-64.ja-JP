...
* キャッシュは、パフォーマンスと正確性の面で、Webサイトがパーソナライズされたコンテンツをどの程度迅速に配信するかという点で、常に最新の状態に保たれます。
* パーソナライゼーションを設定する場合、キャッシュは重要な考慮事項です。時間をかけて検討し、適切な実装がおこなわれるようにしてください。

>[!TIP]
>
>パフォーマンスに対するパーソナライゼーションの影響と関連するキャッシュのトピックについては、[パフォーマンスの最適化](/help/sites-deploying/configuring-performance.md)で詳しく説明します。
...
* したがって、ルールは、ユーザーの要件をフォアグラウンドに置いて十分に考慮する必要があります。 この作業は手間がかかりますが、軽視すべきではありません。パーソナライゼーションの実装では多くの場合、テクニカルな作業よりビジネスルールを定義することが優先されます。

#### {#when-to-use}を使用するタイミング

* Web 上の多くの機能と同じように、パーソナライゼーションは慎重に実行する必要があります。この機能は本当にユーザーの役に立つのか、あるいは、より少ない労力で目的のページへ到達できる方法が他にないか、常に最初に考慮するべきです。パーソナライゼーションは、実際にはメリットがないので、1回だけ設定する機能（機能を確認するため）のリスクを1回だけ実行します。
* パーソナライゼーションは、コンテンツが動的な場合にのみ意味があり、ユーザーによって何らかの意味があります。 すべてのユーザーに同じコンテンツが表示される場合、パーソナライゼーションは不必要です。
