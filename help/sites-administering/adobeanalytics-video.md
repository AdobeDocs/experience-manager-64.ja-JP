...
1. CQ 変数を Adobe Analytics プロパティにマッピングするには、CQ 変数の横のコンテンツファインダーから Adobe Analytics プロパティをコンポーネント上にドラッグします。
   ...
   マッピングの最適化については、[Adobe Analytics でのビデオの測定](https://experienceleague.adobe.com/docs/media-analytics/using/sdk-implement/setup/setup-overview.html)ガイドを参照してください。
