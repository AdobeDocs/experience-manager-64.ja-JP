...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>ルートパス</td> 
   <td>Bulk Editorが検索するルートパスを示します。<br />例えば、<code>/content/geometrixx/en</code>。Bulk Editorは、すべての子ノードを検索します。</td> 
  </tr> 
  <tr> 
   <td>クエリーのパラメーター</td> 
   <td>GQLパラメーターを使用して、Bulk Editorでリポジトリ内で検索する検索文字列を入力します。例えば、<code>type:Page</code>はルートパス内のすべてのページを検索し、<code>text:professional</code>は「professional」という単語を含むすべてのページを検索し、<code>"jcr:title":English</code>は「English」をタイトルとして含むすべてのページを検索します。 文字列のみを検索できます。</td> 
  </tr> 
  <tr> 
   <td>「コンテンツモード」チェックボックス</td> 
   <td>検索結果の<code>jcr:content</code>サブノード内にあるプロパティが存在する場合は、このチェックボックスを選択します。 ページにのみ使用します。プロパティ名の前には <code>"jcr:content/"</code></td> 
  </tr> 
  <tr> 
   <td>プロパティ／列</td> 
  ...
  <tr> 
   <td>カスタムプロパティ／列</td> 
   <td>「<strong>プロパティ/列</strong>」フィールドに表示されていない他のプロパティを入力します。これらのカスタムプロパティは、結果ペインに表示されます。複数のプロパティを追加する場合は、コンマでプロパティを区切ります。 <i>注意：</i> まだ存在しないカスタムプロパティを追加すると、AEM WCMでは空のセルが表示されます。空白のセルを変更して保存すると、そのプロパティがノードに追加されます。新しく作成されたプロパティは、ノードタイプの制約とプロパティの名前空間に従う必要があります。</td> 
  </tr> 
 </tbody> 
</table>
...
1. 「**検索**」をクリックします。結果がBulk Editorに表示されます。
...
* **path:** このパスの下にあるノードのみを検索します。パスのプレフィックスを含む複数のキーワードを指定した場合は、最後のキーワードのみが考慮されます。
* **type:** 指定されたノードタイプのノードのみを返します。これには、プライマリタイプとMixinタイプが含まれます。複数のノードタイプは、コンマで区切って指定できます。GQLは、指定された型のノードを返します。
* **order:** 指定されたプロパティで結果を並べ替えます。複数のプロパティ名は、コンマで区切って指定できます。結果を降順で並べ替えるには、プロパティ名の前にマイナス記号を付けます。例：order:-name.プラス記号を使用すると、結果が昇順で返されます（デフォルトも同様です）。
* **制限：** 結果の数を一定の間隔で制限します。例：limit:10..20間隔はゼロから始まり、startは包含的で、endは排他的であることに注意してください。また、open interval:limit:10..またはlimit:..20を指定することもできます。ドットを省略し、値を1つだけ指定した場合、GQLは最大でこの数の結果を返します。例：limit:10（最初の10件の結果を返します）
...
デフォルトでは、Bulk Editorを開くとインポート機能が非表示になります。 パラメーター`hib=false`をURLに追加するだけで、 Bulk Editorページに「**インポート**」ボタンが表示されます。 コンテンツは、任意のタブ区切りファイル(`.tsv`)から読み込むことができます。 インポートが正しく機能するには、列見出し（セルの最初の行）が、インポート先のテーブルの列見出しと一致している必要があります。
