...
description: AEMのプロキシサーバーツールについて説明します。
...
たとえば、TCP/IP ネットワークを介して通信する 2 つのアプリケーション間にプロキシサーバーを配置できます。例： Web ブラウザーとAEM。 これにより、AEMページをリクエストした際の動作を正確に監視できます。
...
* **b（バイナリモード）** トラフィックで特定のバイトの組み合わせを探している場合は、バイナリモードを有効にします。 次に、16 進数と文字の出力が出力されます。
...
* **i &lt;numindentions> （インデントの追加）** アクティブな各接続は、読みやすくするためにインデントされます。 初期設定は 16 レベルです。 （proxy.jar バージョン 1.16 の新機能）。
...
**キープアライブ** は、クライアントがサーバーへの接続を再利用して、複数のファイル（ページコード、画像、スタイルシートなど）を転送することを意味します。 キープアライブを使用しない場合、クライアントはリクエストごとに新しい接続を確立する必要があります。
...
1. proxy.jar を起動します。
1. 待つか、アクセスログをファイルに書き込みます。各エントリにタイムスタンプが付きます。
1. リクエストがハングし始めると、開いている接続の数と、問題を引き起こしているリクエストを確認できます。
...
プロキシサーバーなしでサーバー（）にアクセスできますが、 経由でアクセスすると、プロキシサーバーは通信をログに記録します。ブラウザーを開き、上記のテンプレートを使用して作成されたページにアクセスしてください。その後、ログファイルを確認します。
...
* JSESSIONID という cookie&lt;%@ page session=&quot;false&quot; %> を使用して明示的にオフにしない場合、これは JSP によって自動的に作成されます。
...
実際には、ページで、画像、スタイルシート、JavaScript ファイルなどに対して、多数の並列リクエストが生成される場合があります。 つまり、ログには、並列オープン接続の重複するエントリが存在します。 この場合、読みやすさを向上させるために、-i オプションを使用することをお勧めします。
