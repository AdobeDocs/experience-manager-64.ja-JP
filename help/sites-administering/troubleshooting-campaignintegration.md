...
HTML ソースをチェックし、クライアントマシンから URL を開くことができることを検証します。URL に localhost:4503 が含まれている場合、オーサーインスタンス上の Day CQ Link Externalizer の設定を、Adobe Campaignコンソールマシンからアクセスできるパブリッシュインスタンスを指すように変更します。

詳しくは、 [Externalizer を設定します。](/help/sites-administering/campaignstandard.md#configuring-the-externalizer)
...
Apache HTTPD サービスを開始していて、エラーが表示される場合 `"Warning: setlocale: LC_CTYPE cannot change locale"` 自分が **en_CA.ISO-8859-15 ロケール** をシステムにインストールします。
...
1. 変更 `label : [inclView.@label](mailto:inclView.@label)` から `label : String([inclView.@label](mailto:inclView.@label))`
...
次の場合、 **同期** ボタンがAdobe Campaign Classicに表示される場合、次のエラーが発生します。
...
* この *Apache Felix Http Service SSL Filter* OSGi 設定 ([https://&lt;host>:&lt;port>/system/console/configMgr](http://localhost:4502/system/console/configMgr)) は、それぞれのヘッダー設定に合わせて設定する必要があります。 詳しくは、 [https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter](https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter)
...
カスタムテンプレートを使用する場合、ログに「com.day.cq.mcm.campaign.servlets.util.ParameterMapper」というエラーが表示されます。 この場合は、[パッケージ共有](/help/sites-administering/package-manager.md#package-share)からフィーチャーパック 6576 をインストールしてください。これは、acMapping プロパティが recipient.firstName 以外の値に設定されている場合、Adobe Campaign Manager 側で空の値が作成される問題です。
