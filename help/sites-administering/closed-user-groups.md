...
|---|---|---|---|
| 可 | 可 | 可 | 有効な権限評価でアクセスが許可されている場合、特定のユーザーがCUGポリシーでマークされたサブツリーの表示のみを実行できます。 認証されていないユーザーは、指定されたログインページにリダイレクトされます。 |
...
ライブコピーに関連した CUG の設定は、以下のように 1 つの追加ノードと 1 つの追加プロパティーを追加することによってリポジトリーに表示されます。
