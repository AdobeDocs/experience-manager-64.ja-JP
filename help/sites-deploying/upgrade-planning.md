...
AEMバージョン 6.0 および 6.4 まで直接アップグレードできます。5.6.x 以前を実行しているお客様は、最初にバージョン 6.0 以降にアップグレードする必要があります。6.0(SP3) をお勧めします。 また、6.3 以降は新しい OAK Segment Tar 形式がセグメントノードストアに対して使用されており、この新しい形式へのリポジトリ移行は 6.0、6.1 および 6.2 でも必須です。
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>中</td> 
   <td><code>/libs</code> および <code>/apps</code> は、アップグレードを通じて簡単に処理できますが、 <code>/etc</code> 通常は、カスタマイズの手動での再適用が必要です。</td> 
  </tr>
  <tr>
   <td>AEM サービス</td> 
   <td>小</td> 
...
AEM 6.4 と互換性を持たせるための[コードのアップグレードとカスタマイズ](/help/sites-deploying/upgrading-code-and-customizations.md)の手順が提供されています。この反復プロセスが実行されるので、必要に応じてランブックを変更する必要があります。 関連トピック [AEM 6.4 の後方互換性](/help/sites-deploying/backward-compatibility.md) ほとんどの場合にアップグレード後に開発を行う必要なく、カスタマイズの後方互換性を維持する方法に関する情報を参照してください。
