...
* **Adobe CQ Web Console**
   ...
   詳細と推 [奨プラクティスについては](/help/sites-deploying/configuring-osgi.md) 、「OSGiの設定」を参照してください。
...
リポジトリデータストアは、大きなバイナリの格納を別の領域に固有のリポジトリからオフロードするために使用され、リポジトリツリー内の同じバイナリ（例えばイメージ）の複数のインスタンスが1回だけ格納されます。
...
詳しくは [PortalとPortlets](/help/sites-administering/aem-as-portal.md) を参照してください。
...
1. AEMプロセスで使用している開いているファイルの数を確認します。
   ...
   このチェックを行う方法は、インスタンスが実行しているプラットフォームによって異なります。 lsof(Unix)やProcess Explorer(Windows)などのユーティリティを使用できます。

   この値は、開発およびテスト中に監視し、次の目的で使用する必要があります。
...
1. 許可する最大値を設定します。
   ...
   この新しい値は、現在のニーズと将来のピークの両方に適している必要があるので、現在のニーズを2倍にすることをお勧めします。

   デフォルトでは、は `serverctl` 次のよ `CQ_MAX_OPEN_FILES` うに設定しま `8192`す。これは、ほとんどのシナリオで十分です。
