...
* **リポジトリ**
   ...
   OSGi 設定のサブセットは、リポジトリで使用できます。 そのため、リポジトリのコンテンツをコピーまたはレプリケートすることにより、同一の設定を再作成できます。実行モードに応じて、独自の設定をリポジトリに追加することもできます。
...
LDAP 認証はリポジトリレベルでおこなわれるので、リポジトリによって直接処理されます。詳しくは、 [AEMでの LDAP の設定](/help/sites-administering/ldap-config.md).
...
AEM Doc Services とAEM Doc Security のリリースに伴い、LiveCycleドキュメントサービスを呼び出して XFA フォームをレンダリングし、ドキュメントをPDFに変換してドキュメントをポリシーで保護できるようになりました。お読みください [AEMLiveCycleコネクタ](https://helpx.adobe.com/livecycle/help/aem/aem-livecycle-connector.html) を参照してください。
...
リポジトリのデータストアは、大きなバイナリのストレージをリポジトリから別の領域に適切にオフロードするために使用され、リポジトリツリー内の同じバイナリの複数のインスタンス（画像など）が 1 回だけ保存されます。
...
詳しくは、 [ポータルとポートレット](/help/sites-administering/aem-as-portal.md) 詳しくは、を参照してください。
...
1. AEMプロセスで使用している開いているファイルの数を確認します。
   ...
   この値は、以下をおこなうための開発およびテスト時に監視する必要があります。
...
1. 許可する最大値を設定します。
   ...
   デフォルトでは、 `serverctl` 設定 `CQ_MAX_OPEN_FILES` から `8192`;これは、ほとんどのシナリオで十分です。
...
* オーサーインスタンス上：
   ...
   * [Adobeページインプレッション数の追跡](/help/sites-deploying/osgi-configuration-settings.md)
