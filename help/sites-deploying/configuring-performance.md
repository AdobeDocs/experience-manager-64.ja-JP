...
この環境には、ユーザーが利用できるようにするコンテンツが含まれています。ここでは、リクエストの数はさらに多く、速度も非常に重要ですが、リクエストの性質は動的ではないので、パフォーマンス向上メカニズムを追加で適用できます。例えば、コンテンツのキャッシュやロードバランシングなどを行います。
...
これらのルールは、Webプロジェクト全般に適用され、プロジェクトマネージャーやシステム管理者に関連しており、起動時にプロジェクトのパフォーマンスが低下しないようにします。
...
可能な限り、実際のエクスペリエンスを収集し、さらに最適化を行うために、限られた対象者に対してプロジェクトをソフト起動する必要があります。その際、発表後の圧力は増えません。
...
現実をシミュレートするのは難しく、「現実」を手に入れるのに合理的にどれだけの努力を費やすかは、プロジェクトの性質に依存します。「Real」とは、単に「real code」と「real traffic」だけでなく、特にコンテンツのサイズと構造に関して「real content」を意味します。テンプレートは、リポジトリのサイズと構造によって動作が完全に異なる場合があることに注意してください。

### 適切な目標設定 {#establish-solid-goals}

![chlimage_1-5](assets/chlimage_1-5.jpeg)

パフォーマンス目標を適切に設定することの重要性は、過小評価してはいけません。多くの場合、人々が特定のパフォーマンス目標に集中したら、狂った前提に基づいていても、後でこれらの目標を変更するのは非常に困難です。
...
パフォーマンス調整は、目標に達するまでの測定、分析、最適化、検証を含む反復的なプロセスです。この側面を適切に考慮するために、反復のたびにより重量の多いテストプロセスではなく、最適化フェーズでの迅速な検証プロセスを実装します。
...
* CPU - 複数のコアがあると、トランスコーディングの際に処理がスムーズにおこなわれます。
...
ワークフローエンジンは、作業項目の処理とスケジュールの設定にApache Slingジョブキューを使用します。次のジョブキューサービスは、ワークフロージョブを処理するためのApache Sling Job Queue Configurationサービスファクトリからデフォルトで作成されました。

* Graniteワークフローキュー：DAMアセットを処理するワークフローステップなど、ほとんどのワークフローステップでは、Granite Workflow queueサービスを使用します。
...
**** 注意：これらのジョブキューの設定は、特定のワークフローモデル用のジョブキューを作成していない限り、すべてのワークフローに影響します(以下の「特定のワーク [フローモデル用のキューの設定](/help/sites-deploying/configuring-performance.md#configure-the-queue-for-a-specific-workflow) 」を参照)。
...
Dispatcher バージョン 4.1.11 では、応答ヘッダーをキャッシュできます。Dispatcher で応答ヘッダーをキャッシュしない場合は、MIME タイプが HTTP ヘッダーに含まれていることに注意してください。したがって、AEMアプリケーションが、認識されるファイルの終わりがなく、MIMEタイプに依存するファイルを返す場合、これらのファイルが正しく表示されない可能性があります。
...
**アイドル状態のバックアップは** 、CQ上で他のアクティビティを実行せずに実行されます。

**「Load** Backups」では、システムがオンライン・プロセスから80%の負荷を受けている間に実行されます。 負荷に対する影響を確認するためにバックアップ遅延を変更しました。
...
**バックアップ** ・タイプ：すべてのバックアップは、 zipを作成せずにバックアップ・ディレクトリに対して作成されたリポジトリの外部バックアップでした。ただし、 tarコマンドが直接使用された場合の比較を除きます。 増分バックアップはzipファイルには作成できないので、また、以前の完全バックアップがzipファイルの場合は、バックアップディレクトリ方式が実稼働環境で最も頻繁に使用されます。
