...
1. JMX コンソールに移動し、 **org.apache.jackrabbit.oak:ステータス（「スタンバイ」）** 豆。 この方法について詳しくは、 [監視](#monitoring).
...
1. メンテナンスプロセスが完了し、インスタンスが再起動したら、次の手順を実行します。
   ...
   * スタンバイ状態では、データストアのガベージコレクションは、 **BlobGarbageCollection** MBean - `startBlobGC()`. この **RepositoryManagement** MBean はスタンバイでは使用できません。
