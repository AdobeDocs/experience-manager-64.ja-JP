...
1. If AEM is already configured to work with the Tar or MongoDB storage, remove any existing configuration files from the `aem-install/crx-quickstart/install` folder before proceeding. 削除する必要があるファイルは次のとおりです。
