...
* 優先度とコ **ンテキストの実装をサポート：**
   ...
   WCAG 1.0と同様に、障害を持つ特定のユーザーグループに対してガイドラインに従わない場合の影響に関して、WCAG 2.0のガイドラインに優先順位を付けます。 これにより、作成者は、特定の状況下で最も重要となるガイドラインを十分な情報から判断することができます。さらに、アクセシビリティサポート&#x200B;**&#x200B;の概念が導入されています。そのため、作成者は、アクセシビリティサポートが完全ではない Web テクノロジーを最大限に生かす方法を判断したり、アクセシビリティ機能の恩恵を受けられるように、特定の支援テクノロジーまたはブラウザーを使用するようユーザーに要求したりすることができます。
...
WCAG 2.0は、アクセシブルなデザインのための4つの主要な原則で構成されています。 これらは次のとおりです。
...
* コレクション別（一般的な手法、またはHTML、CSS、クライアントサイドスクリプティングなどの特定の技術や形式に関する手法）、または
...
* 手法と失敗：成功基準の満たし方の具体的で詳細な例（以下で詳しく説明）
* 主要用語 — 成功基準を理解する上で重要な用語集です。
