...
* [スクリーン](/help/screens/aem-screens-introduction.md)
...
* **関与**
   ...
   * お客様側では、作成者（日々システムを使用する必要がある作成者）も含まれます。
   * 独自のプロジェクトチーム内では、品質保証の責任者も含まれます。 お客様の要件を理解すればするほど、テストの計画がより適切になります。

* **伝達経路**

   * これらを過度に正式化すべきではありませんが、具体的な定義によって、主要な人々に常に通知され、その結果最新の情報が保たれるようにする必要があります。 外部の関係者とのコミュニケーションには、特に注意が必要です。

* **プロセス**

   定義するプロセスは、個々のプロジェクトに依存します。 もう一度、以下の点を考慮して、これらのシンプルさを保つようにします。

   * 第三者と対話するためのプロセス（および通信のパス）の定義例えば、デザイン代理店やサードパーティのソフトウェアサプライヤーなど。
...
* **追跡ツール**
   ...
   * ここで注意すべき重要な点は、情報のコピーを1つだけ保持し、情報を共有することです（したがって、使用中のツールにアクセスすることです）。 これにより、メンテナンスが容易になり、不一致を防ぐのに役立ちます。

* **対象範囲**

   様々なレベルで、プロジェクトが対象とする内容を明確に定義します。

   * 個々のリリース（反復的なリリースプロセスが使用されている場合、顧客または社内テストチームに配信されるかどうかに関係なく）。
   * AEM プロジェクト。
   * プロジェクト全体サードパーティ製ソフトウェア、テスト、組織の問題、その他多くの問題に対する影響を含みます。
...
この情報は、プロジェクトハンドブック内で定義できます。また、Wiki の使用も、継続的な変化に効率よく対応するために役立ちます。これらが定義されている場合、主な要因は次のとおりです。

* 情報の定義と維持
* 関係者全員に情報が明確に伝えられる。 標準的なプロジェクト管理方法ではありますが、役割定義が明確であるかや、伝達が良好であるかによって、プロジェクトの成否が左右されるということを繰り返し肝に銘じてください。
...
Target指標は、プロジェクトの開始時に次の目的で定義されます。

* 提供するウェブサイトの期待されるサイズを示す
* 達成したい最低品質を示す
* これらの要因が実際どのように測定されるかを定義する
* [主要業績評価指標](#key-performance-indicators-and-target-metrics)の基礎として使用される

ターゲット指標を定義する際は、常に注意が必要です。

* 高く設定しすぎると、完全に到達できない可能性があります
* 低い揺れを設定しすぎると、強調表示されない場合があります。
* 繰り返し一貫して測定できるようにする
* 測定される異なる要因間のバランスを提供する
* 特定の指標はテスト環境に関連しますが、実稼働用Webサイトで測定可能で複製可能である必要があるので、一部の指標は実稼働用シナリオを反映する必要があります
* ウェブサイトに対する指標の重要性に基づいて指標の優先順位を付ける
* 現実的に監視できる指標に限定する
...
適切に使用すると、これらの指標が役に立つツールを提供します。無責任に使うと、時間を無駄にする気晴らしになる。 いつものように、測定するもの、測るもの、測定するもの、そしてその理由を理解する必要があります。

>[!NOTE]
>
>本節では、検討すべき基本原則及び課題について取り組む。 各インストール環境は異なるので、実際の測定値は異なります。

### すべての基礎となるプロジェクトデザイン {#everything-rests-on-your-project-design}

測定されるすべての指標は、ある意味で、プロジェクトのデザインの影響を受けます。 逆に、多くの問題は設計の変更によって解決するのが最適です。

したがって、デザインを決定する&#x200B;*前に*&#x200B;ターゲットメトリックを定義する必要があります。これにより、これらの要因に基づいてデザインを最適化できます。 プロジェクトが開発されると、基本的な設計原則を変更するのは難しくなります。

Web サイトの構造を作成する際には、AEM Web サイトで推奨される構造に従います。以下の問題や原則を理解していることを確認します。
...
* パーソナライズされたコンテンツの影響。
* 検索機能の仕組み
* CSSと関連テクノロジーを使用して、コンパクトで非冗長なHTMLコードを作成する方法。

デザインがガイドラインに従っていないと感じる場合や、意味が不明な場合は、プログラミング段階を開始する前に、またはコンテンツを入力する前に、これらの問題を明確にしてください。
...
* **オーサー環境**
   ...
   * コンテンツページとそのページ上の個々の要素を更新する際に、大量のリクエストを生成する少数のユーザーを対象とする
   * コンテンツを Web サイトにできるだけ早く展開し、生産性を最大限に高める

* **パブリッシュ環境**

   この環境には、ユーザーが利用できるようにするコンテンツが含まれています。

   * 速度は重要であるが、多くの場合、オーサー環境より遅くなる
   * パフォーマンス向上メカニズムが多く適用されます。
...
* ページに対する要求の70%は、100 ms未満で応答する必要があります。
* ページに対するリクエストの25%は、100ms～300ms未満で応答する必要があります。
* ページのリクエストの4 %が300ミリ秒～ 500ミリ秒未満で応答する必要があります。
* ページのリクエストの1 %が500ミリ秒～ 1000ミリ秒未満で応答する必要があります。
...
* **AEM request.log を使用した応答時間の監視**
   ...
   パフォーマンス分析の良い開始点は、要求ログです。 その他の情報としては、個々のリクエストの応答時間を確認する場合に使用できます。 詳しくは、 [「パフォーマンスの最適化](/help/sites-deploying/configuring-performance.md) 」を参照してください。
...
* **オーサー環境**
   ...
   * 通常、同時ユーザ数を正確に見積もることができます。 合計で何名の発言者がいるかはわかりますが、同時にすべての発言者がアクティブになるわけではありません。

* **パブリッシュ環境**

   * この方が予測が難しいので、ターゲット値を選択する必要があります。 これは、現在のWebサイトの経験と、新しいWebサイトに対する現実的な期待に基づくものです。
...
* **処理能力**
   ...
   * システムがボリュームを提供する能力です。
   * 各ステップで、容量とボリュームの測定方法が異なります。次の表を参照してください。 最高のパフォーマンスを得るには、各ステップのボリュームと容量が一致し、容量とボリュームがすべてのステップで共有されていることを確認します。 例えば、要求ごとにサーバーでナビゲーションを計算する代わりに、クライアントコンピューターで計算したり、キャッシュに配置したりできます。
...
* **処理能力とボリューム**
   ...
   | ディスパッチャーキャッシュ | ウェブサーバのサーバメモリ（メインメモリとハードドライブ）。 | Webサーバー（メインメモリとハードドライブ）。 キャッシュされたページの数とサイズ。 |
   | 出力キャッシュ | AEMサーバーのサーバーメモリ（メインメモリおよびハードドライブ）。 | 出力キャッシュ内のページの数とサイズ、ページあたりの依存関係の数。 Dispatcher キャッシュを使用すると、このボリュームが少なくなります。 |
   | Web サーバー | ウェブサーバの計算能力。 | リクエストの量。 キャッシュを使用すると、このボリュームが少なくなります。 |
   | テンプレート | ウェブサーバの計算能力。 | テンプレートの複雑さ。 |
...
具体的な要件に応じて、分離して追加の指標を定義したり、上記の分類を考慮したりすると役に立つ場合があります。

ただし、Webサイトのあらゆる側面を測定および定義するのではなく、簡単かつ確実に機能する正確なコア指標の小さなセットを用意することをお勧めします。 Webサイトは、その性質上、ユーザに提供され次第、変化し始め、進化し始めます。
...
結果のタスクリストに応じて、（高レベルの）タスク定義に関する時間と労力の初期見積もりを作成できます。 これには、（顧客またはパートナー）が何をいつ実行するかを示す指標が含まれます。
...
<table>
 <tbody>
  ...
  <tr>
   <td>AEM</td>
   <td><p>AEM自体は、アプリケーションの監視、テスト、調査、デバッグを行うための様々なメカニズムを提供します。含める：</p>
    <ul>
     ...
     <li>コンテンツ <a href="/help/sites-authoring/author-environment-tools.md#content-tree">ツリー</a></li>
    </ul> </td>
  </tr>
  <tr>
   <td> </td>
