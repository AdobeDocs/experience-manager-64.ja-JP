...
* ユーザーにとって URL がわかりやすいほど、効果的です。URL にキーワードを含めると、価値が高まります。
   ...
   * 例：～する `mybrand.com/products/product-detail.product-category.product-name.html` のが好まれる `mybrand.com/products/product-detail.1234.html`
...
作成者が、プロモーション目的で別の場所からアクセス可能なページを作成する場合、ページごとに定義される AEM のバニティー URL が役立つことがあります。To add a vanity URL for a page, navigate to it in the **[!UICONTROL Sites]** console and edit the page properties. At the bottom of the **[!UICONTROL Basic]** tab, you see a section where vanity URLs can be added. 複数のURLを介してページにアクセスできるように設定すると、ページのSEO値がフラグメント化されるので、この問題を回避するために、正規のURLタグをページに追加する必要があります。

#### ページ名のローカライズ {#localized-page-names}

翻訳されたコンテンツのユーザーに、ローカライズしたページ名を表示できます。次に例を示します。

* スペイン語を話すユーザーが次の場所に移動するのではなく、
...
>[!NOTE]
>
> このプロ `sling:alias` パティは、ページプロパティの編集時に [Aliasプロパティを使用して設定できます](/help/sites-authoring/editing-page-properties.md#advanced)
...
これまでのところ、ページにURLを出力する際にこれらのマッピングを使用するように、コンポーネント内のロジックと共にマッピングを実装してきました。
...
最善の方法は、小文字を使用してすべてのページを提供することです。ただし、ユーザーが URL に大文字を使用して Web サイトにアクセスしたときに、404 エラーが表示されるのは望ましくありません。このため、アドビでは、Apache HTTP server設定に書き換えルールを追加して、すべての受信URLを小文字にマップすることをお勧めします。 さらに、小文字の名前を使用してページを作成するようコンテンツの作成者をトレーニングする必要があります。
...
検索エンジンでは、サイトをクロールする前に、サイトのルートに *ファイルがあるかどうかがチェックされる*`robots.txt`はずです。Google、Yahoo、Bingなどの主要検索エンジンはこれらをすべて尊重しますが、一部の外国検索エンジンはこの点を重視しないので、Shoudはここで強調する必要があります。
...
>[!NOTE]
>
>You can register a Sling Servlet to listen for the selector `sitemap` with the extension `xml`. これにより、次の語句で終わるURLが要求された場合に、サーブレットは要求を処理します。
>
...
新しい構造でサイトを起動する場合、Apache HTTP Serverで301リダイレクトを実装し、テストすることが重要です。理由は2つあります。
