...
このサイジングのガイドラインでは、AEM プロジェクトのデプロイに必要なハードウェアリソースの概算値を示します。サイズの見積もりは、プロジェクトのアーキテクチャ、ソリューションの複雑さ、予想トラフィック、およびプロジェクトの要件によって異なります。 このガイドは、特定のソリューションに必要なハードウェアや、ハードウェア要件の上限、下限の概算値を把握する際に役立ちます。
...
* **作成者テスト環境**&#x200B;変更を検証します。 テスト環境の数は、プロジェクトの要件に応じて異なる場合があります（例えば、QA、統合テスト、ユーザー受け入れテスト用に別々に行います）。
...
オンライン、オフラインおよびリビジョンのクリーンアップ中は、ディスク容量が継続的に監視されます。使用可能なディスク容量が臨界値を下回ると、プロセスがキャンセルされます。臨界値はリポジトリの現在のディスクフットプリントの 25 ％で、設定はできません。推定増加量を含め、リポジトリ・サイズの少なくとも2倍または3倍のサイズでディスクをサイズ設定することをお勧めします。
...
フェールセーフWebサイトは、少なくとも2つの別々のシステムに展開されます。 一方のシステムが故障した場合、他方のシステムが引き継ぎ、システム障害を補うことができます。
...
* **ベンチマークテスト1**
   ...
   ユーザーが基本負荷300の既存ページに加えて単純なページ作成の演習を行うロードプロファイルの最大スループットを計算します。 実行した手順は、サイトへのログイン、SWF と画像／テキストを使用したページの作成、タグクラウドの追加、ページのアクティベーションです。

   * **結果**

      上記のような単純なページ作成の演習（1トランザクションと見なされる）の最大スループットは、1時間あたり1730トランザクションであることが判明しました。

* **ベンチマークテスト2**

   ロードプロファイルに新規ページの作成(10%)、既存ページの変更(80%)、ページの作成後の変更(10%)が混在する場合の最大スループットを計算します。 ページの複雑度はベンチマークテスト 1 のプロファイルの場合と同じです。ページの基本的な変更として、画像を追加し、テキストコンテンツを変更します。この場合も、ベンチマークテスト 1 に定義されたものと同じ複雑度を持つ 300 個のページを基本負荷として、作業をおこないました。
...
* **ベンチマークテスト1a**
   ...
   2つのオーサーインスタンスのアクティブ — アクティブ共有 — なしクラスターを使用して、基本負荷300の既存ページに対する単純なページ作成の演習を行うロードプロファイルを使用して、最大スループットを計算します。

   * **結果**

      上記のような単純なページ作成の演習での最大スループット（1トランザクションと見なされる）は、2016年のトランザクション/時間です。 スタンドアロンのオーサーインスタンスに対して同じベンチマークテストを実行した場合と比較して、約 16 ％増えています。

* **ベンチマークテスト2b**

   2つのオーサーインスタンスのアクティブ — アクティブ共有 — なしクラスターを使用して、読み込みプロファイルに新規ページ作成(10%)、既存ページの変更(80%)、ページの作成と変更(10%)の組み合わせがある場合の最大スループットを計算します。 ページの複雑度はベンチマークテスト 1 のプロファイルの場合と同じです。ページの基本的な変更として、画像を追加し、テキストコンテンツを変更します。この場合も、ベンチマークテスト 1 に定義されたものと同じ複雑度を持つ 300 個のページを基本負荷として、作業をおこないました。

   * **結果**

      このような混在操作シナリオの最大スループットは、1時間あたり6288トランザクションであることが判明しました。 スタンドアロンのオーサーインスタンスに対して同じベンチマークテストを実行した場合と比較して、約 93 ％増えています。
...
アドビのベンチマークテストは、Hewlett-Packard proLiant DL380 G5ハードウェアプラットフォーム上で実行され、次の設定を使用してRedHat 5.5オペレーティングシステムを使用して実行されました。
...
複雑さに基づいて、パブリッシュ環境に必要なサーバー（またはCPUコア）の数を次のように判断できます。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>単純なアプリケーションを 1、複雑なアプリケーションを 2 として、1 ～ 2 の値を使用します。</p>
    <ul>
     <li>1 — 完全に匿名の、コンテンツ指向サイト</li>
     <li>1.1 — 完全に匿名の、コンテンツ指向サイトで、クライアント側/Targetのパーソナライゼーションを使用</li>
     <li>1.5 — 匿名セクションとログインセクション、クライアント側/Targetのパーソナライゼーションを備えたコンテンツ指向サイト</li>
     <li>1.7 — 匿名セクションとログインセクションの両方を持つコンテンツ指向サイトの場合、クライアント側/Targetのパーソナライゼーションと、一部のユーザー生成コンテンツ</li>
     <li>2 — サイト全体でログインが必要な場所で、ユーザー生成コンテンツを広範に使用し、様々なパーソナライゼーション技術を使用</li>
    </ul> </td>
  </tr>
  <tr>
   <td>cacheRatio</td>
  ...
  <tr>
   <td>activations</td>
   <td>1時間あたりの平均アクティベーション数（作成者から発行層への平均サイズのページとアセットの複製）をxで割った数です。xは、システムで処理される他のタスクに対するパフォーマンスに影響を与えない、システム上で実行されたアクティベーションの数です。 x = 100 のように、悲観的な初期値をあらかじめ定義しておくこともできます。<br /> </td>
  </tr>
 </tbody>
</table>
