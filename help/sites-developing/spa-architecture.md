...
開発者は、コンテンツ構造を参照する静的パスを使用しないでください。パスは、コンテンツ作成者がいつでも変更できるからです。 また、これによりライブラリの再利用が制限され、AEMテンプレートエディターの構造がコンテンツ以外の場所に配置されるので、AEMテンプレートエディターを使用できなくなります。
...
AEM [でのSPAの開発の](/help/sites-developing/spa-architecture.md#spa-development-principles-for-aem) 原則に従う場合は、SPAが、サポートされるすべてのAEMコンテンツオーサリング機能と共に機能するようになります。
