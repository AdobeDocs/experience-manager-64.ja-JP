...
* **`Blueprint`**
「 `Blueprint` ( [Blueprint設定のように](/help/sites-administering/msm.md#source-blueprints-and-blueprint-configurations))」は、ライブコピーがコンテンツを継承できるページを指定します。
...
* **`LiveActionFactory`** 設定が与えられた `LiveAction` オブジェクトを作成 `LiveAction` します。 設定は、リポジトリ内にリソースとして保存されます。

* **`RolloutConfig`** は、トリガされたときに使用さ `LiveActions`れる、のリストを保持します。 The `LiveCopy` inherits the `RolloutConfig` and the result is present in the `LiveRelationship`.
