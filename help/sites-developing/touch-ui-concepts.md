...
>[!NOTE]
>
>タッチ対応UIはAEMの標準UIですが、従来のUIは引き続きサポートされます。
...
タッチ操作対応UI構造の詳しい概要については、「AEMタッチ対応UI [の構造」の記事を参照してください](/help/sites-developing/touch-ui-structure.md)。
...
* 自然の中では一般的です For example, a layout manager provides layout for any list of `DIV` or `LI` elements
...
* `data-*` 属性を参照してください。 例えば、列数を指定するには
...
サンプルマークアップの抽出（data-&amp;ast；として指定されたオプションに注意）属性):
