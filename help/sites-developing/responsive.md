...
>[!NOTE]
>
>Adobeは、単一ページアプリケーションのフレームワークベースのクライアントサイドレンダリング ( _React_) をクリックします。 [詳細情報](/help/sites-developing/spa-overview.md)を参照してください。
...
デザイン時に、 **[!UICONTROL サイドキック]** ページを様々な画面サイズでプレビューできます。
...
We.Retail Media サンプルではこの実装方法を使用して、サイトデザインのスタイルを定義しています。We.Retail で使用される CSS ファイルは、次の場所にあります。 `*/apps/weretail/clientlibs/clientlib-site/less/grid.less`.
...
css.txt ファイル ( `/etc/designs/weretail/clientlibs` フォルダーには、クライアントライブラリフォルダーに含まれる CSS ファイルが一覧表示されます。 ファイルの記載順序により、スタイルの優先順位が実装されます。スタイルは、デバイスのサイズが小さくなるほど特化されます。
...
**ヒント**:説明的なファイル名を使用すると、ターゲットのビューポートサイズを簡単に識別できます。
...
>[!NOTE]
>
>この `apps.weretail.all` クライアントライブラリフォルダーに clientlibs ライブラリが埋め込まれました。
...
異なる表示域サイズでページのプレビューを参照して、レスポンシブデザインの動作をテストします。In **[!UICONTROL プレビュー]** モード **[!UICONTROL サイドキック]** を含む **[!UICONTROL デバイス]** をクリックします。 デバイスを選択すると、その表示域サイズに合わせてページが変更されます。

![chlimage_1-5](assets/chlimage_1-5.png)

でデバイスのプレビューを有効にするには **[!UICONTROL サイドキック]**&#x200B;を設定する場合は、ページと **[!UICONTROL MobileEmulatorProvider]** サービス。別のページ設定で、 **[!UICONTROL デバイス]** リスト。

### 「デバイス」リストの追加 {#adding-the-devices-list}

この **[!UICONTROL デバイス]** リストが表示されます **[!UICONTROL サイドキック]** ページに、 **[!UICONTROL デバイス]** リスト。 次の手順で **[!UICONTROL デバイス]** リスト **[!UICONTROL サイドキック]**、 `/libs/wcm/mobile/components/simulator/simulator.jsp` スクリプトを `head` 」セクションに追加します。

この `head` セクションを定義している JSP で次のコードをインクルードします。

`<cq:include script="/libs/wcm/mobile/components/simulator/simulator.jsp"/>`

例を確認するには、 `/apps/weretail/components/page/head.jsp` ファイルをCRXDE Lite
...
* 名前：`com.day.cq.wcm.mobile.core.impl.MobileEmulatorProvider-*alias*`
   ...
   - `*alias*` MobileEmulatorProvider サービスはファクトリサービスなので、サフィックスが必要です。 このファクトリで一意となる任意のエイリアスを使用します。
...
* 型：`String[]`
* 値：Web ページをレンダリングするページコンポーネントへのパス。例えば、geometrixx-media アプリでは次の値が使用されます。
...
「デバイス」リストに表示されるデバイスグループを指定するには、 `cq:deviceGroups` プロパティを `jcr:content` サイトのルートページのノードに設定します。 プロパティの値は、デバイスグループノードへのパスの配列です。

デバイスグループノードは、 `/etc/mobile/groups` フォルダー。

例えば、Geometrixx Mediaサイトのルートページは `/content/geometrixx-media`. この `/content/geometrixx-media/jcr:content` ノードには次のプロパティが含まれます。
...
の実装 [MediaQueryList インターフェイス](https://dev.w3.org/csswg/cssom-view/#the-mediaquerylist-interface) W3C で定義されているので、javascript を使用したメディアクエリを評価できます。メディアクエリの結果にロジックを適用し、現在のウィンドウを対象とするスクリプトを実行できます。

* MediaQueryList インターフェイスを実装するブラウザーは、 `window.matchMedia()` 関数に置き換えます。 この関数は、指定された文字列に対してメディアクエリをテストします。 この関数は、 `MediaQueryList` オブジェクトを返します。

* インターフェイスを実装していないブラウザーの場合、 `matchMedia()` ポリフィル： [matchMedia.js](https://github.com/paulirish/matchMedia.js)：自由に利用できる javascript ライブラリ。
...
無料で利用可能な [picturefill.js ライブラリ](https://github.com/scottjehl/picturefill) は、提案したとおりの機能を提供します `picture` 要素を抽出し、同様の方法を使用します。 picturefill.js ライブラリの呼び出し `window.matchMedia` 一連の `div` 要素。 各 `div` element は、画像のソースも指定します。 ソースは、 `div` 要素の戻り値 `true`.

この `picturefill.js` ライブラリには、次の例のようなHTMLコードが必要です。
...
ページがレンダリングされると、picturefull.js は `img` 要素を `<div data-picture>` 要素：
...
* [matchMedia.js](https://github.com/paulirish/matchMedia.js) （MediaQueryList インターフェイスを実装していないブラウザーの場合）
* [picturefill.js](https://github.com/scottjehl/picturefill)
* jquery.js ( `/etc/clientlibs/granite/jquery` クライアントライブラリフォルダー（カテゴリ= jquery）
* [jquery.debouncedresize.js](https://github.com/louisremi/jquery-smartresize)（ウィンドウのサイズ変更後に一度発生する jquery イベント）

**ヒント：** 複数のクライアントライブラリフォルダーを [埋め込み](/help/sites-developing/clientlibs.md#embedding-code-from-other-libraries).

**HTML**

picturefill.js コードが想定する必要な div 要素を生成するコンポーネントを作成します。AEMページでは、data-src 属性の値は、リポジトリ内のリソースへのパスになります。例えば、ページコンポーネントは、DAM の画像レンディションに関連付けられたメディアクエリやパスをハードコードで記述できます。または、カスタムの画像コンポーネントを作成し、作成者が画像レンディションを選択したり、ランタイムレンダリングオプションを指定したりできるようにします。
...
### AEMでの画像レンダリングについて {#understanding-image-rendering-in-aem}

画像レンダリングをカスタマイズするには、静的画像レンダリングの AEM デフォルト実装を理解する必要があります。AEM は画像コンポーネントと画像レンダリングサーブレットを提供しており、これらが連携して Web ページ用の画像をレンダリングします。画像コンポーネントがページの段落システムに含まれている場合、次の一連のイベントが発生します。
...
この `image_0` ノードに `jcr:resourceType` 値 `foundation/components/image`( `sling:resourceSuperType` 値 `foundation/components/parbase`. parbase コンポーネントには、セレクターと要求 URL のファイル名拡張子に一致する img.GET.java スクリプトが含まれます。 CQ はこのスクリプト（サーブレット）を使用して画像をレンダリングします。

スクリプトのソースコードを確認するには、CRXDE Liteを使用して `/libs/foundation/components/parbase/img.GET.java`\
ファイルに格納されます。

## 現在のビューポートサイズの画像の拡大/縮小 {#scaling-images-for-the-current-viewport-size}
...
アダプティブ画像コンポーネントは、デバイス画面に従ってサイズが設定された画像をレンダリングするために、アダプティブ画像コンポーネントサーブレットへの呼び出しを生成します。 このコンポーネントには次のリソースが含まれています。

* JSP：メディアクエリと Adaptive Image Component Servlet への呼び出しを関連付ける div 要素を追加します。
* クライアントライブラリ：clientlibs フォルダーは、 `cq:ClientLibraryFolder` matchMedia polyfill javascript ライブラリと変更された Picturefill javascript ライブラリをアセンブリします。
* 編集ダイアログボックス：この `cq:editConfig` ノードは、CQ 基盤画像コンポーネントをオーバーライドし、ドロップターゲットが基盤画像コンポーネントではなく、アダプティブ画像コンポーネントを作成するようにします。
...
の値 `data-scr` 属性は、画像をレンダリングするアダプティブ画像コンポーネントサーブレットに解決される URL です。 data-media 属性には、クライアントプロパティに対して評価されるメディアクエリが含まれます。
...
例えば、Sling は URL を持つ HTTP 要求を転送します `http://localhost:4502/content/geometrixx/adaptiveImage.img.jpg` をアダプティブ画像コンポーネントサーブレットに追加します。
...
この `com.day.cq.wcm.foundation.impl.AdaptiveImageComponentServlet` クラスは、 [AbstractImageServlet](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.html) クラス。 AdaptiveImageComponentServlet のソースコードは、 `/libs/foundation/src/impl/src/com/day/cq/wcm/foundation/impl` フォルダー。
...
この `AbstractImageServlet` クラスは `doGet` HTTP リクエストを処理するメソッド。 このメソッドは、リクエストに関連付けられているリソースを決定し、リポジトリからリソースのプロパティを取得して、 [ImageContext](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.ImageContext.html) オブジェクト。

>[!NOTE]
>
>この [com.day.cq.commons.DownloadResource](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/commons/DownloadResource.html) クラスは `getFileReference method`：リソースの `fileReference` プロパティ。

この `AdaptiveImageComponentServlet` クラスは、 `createLayer` メソッド。 このメソッドは、 `ImageContext` オブジェクト。 その後、 `info.geometrixx.commons.impl.AdaptiveImageHelper` クラス。実際の画像の拡大/縮小を実行します。

また、AdaptiveImageComponentServlet クラスは writeLayer メソッドをオーバーライドします。この方法は、画像にJPEG品質を適用します。
...
このサーブレットは `cq:page` リソースにバインドされ、.jpg ファイル拡張子をサポートしています。サーブレットセレクターは、 `image`. そのため、Sling は次の形式の HTTP 要求 URL をこのサーブレットへと解決します。

`path-to-page-node.image.jpg`

例えば、Sling は URL を持つ HTTP 要求を転送します `http://localhost:4502/content/geometrixx/en.image.jpg` を画像参照変更サーブレットに追加します。
...
* ノード名はです。 `image`.
* 親ノードは `jcr:content` ノード `cq:page` リソース。

* 画像のパスは、`fileReference` というプロパティの値として格納します。

ページを作成する場合は、 **サイドキック** 画像を指定し、 `image` ノードからページプロパティへ：

1. In **サイドキック**、 **ページ** 「 」タブで、「 」をクリックします。 **ページプロパティ**.
1. 次をクリック： **画像** 」タブで画像を指定します。
1. 「**OK**」をクリックします。

#### 実装の詳細 {#implementation-details-1}

info.geometrixx.commons.impl.servlets.ImageReferenceModificationServlet クラスは、 [AbstractImageServlet](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.html) クラス。 cq-geometrixx-commons-pkg パッケージがインストールされている場合、ImageReferenceModificationServlet のソースコードは、 `/apps/geometrixx-commons/src/core/src/main/java/info/geometrixx/commons/impl/servlets` フォルダー。
...
この `AbstractImageServlet` クラスは `doGet` HTTP リクエストを処理するメソッド。 このメソッドは、呼び出しに関連付けられるリソースを決定し、リポジトリからリソースプロパティを取得して、それらを [ImageContext](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.ImageContext.html) オブジェクト。

この `ImageReferenceModificationServlet` クラスは、 `createLayer` メソッドを使用し、レンダリングするイメージリソースを決定するロジックを実装します。 メソッドは、ページの `jcr:content` 次のノード名 `image`. An [画像](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/Image.html) オブジェクトはこの `image` ノード、 `getFileReference` メソッドは、 `fileReference` プロパティに含まれます。

>[!NOTE]
>
>この [com.day.cq.commons.DownloadResource](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/commons/DownloadResource.html) クラスは getFileReference メソッドを提供します。

## 可変グリッドの開発 {#developing-a-fluid-grid}

AEMを使用すると、流体グリッドを効率的かつ効果的に実装できます。このページでは、可変グリッドまたは既存のグリッド実装 ( [Bootstrap](https://twitter.github.com/bootstrap/)) をAEMアプリケーションに追加します。
...
例えば、サンプルの geometrixx-media アプリケーションには media-home コンポーネントが含まれています。 このページコンポーネントは、2 つのスクリプトを挿入し、2 つのスクリプトを生成します `div` クラスの要素 `row-fluid`:

* 最初の行には `div` クラスの要素 `span12` （コンテンツは 12 列に及びます）。 この `div` 要素には parsys コンポーネントが含まれます。

* 2 番目の行には、2 つの `div` 要素、クラスの 1 つ `span8` そしてもう一つはクラスの `span4`. それぞれの `div` 要素には parsys コンポーネントが含まれます。
...
>[!NOTE]
>
>コンポーネントに複数の `cq:include` parsys コンポーネントを参照する要素、各 `path` 属性には異なる値が必要です。

#### ページコンポーネントグリッドの拡大/縮小 {#scaling-the-page-component-grid}

geometrixx-media ページコンポーネントに関連付けられているデザイン (`/etc/designs/geometrixx-media`) には、 `clientlibs` ClientLibraryFolder。 この ClientLibraryFolder は、の CSS スタイルを定義します。 `row-fluid` クラス、 `span*` クラスと `span*` の子であるクラス `row-fluid` クラス。 メディアクエリを使用すると、異なるビューポートサイズに対してスタイルを再定義できます。

次の CSS の例は、これらのスタイルのサブセットです。 このサブセットでは、 `span12`, `span8`、および `span4` 2 つのビューポートサイズ用のクラスとメディアクエリ。 CSS の次の特性に注意してください。

* この `.span` スタイルは、絶対数を使用して要素の幅を定義します。
* この `.row-fluid .span*` スタイルは、要素の幅を親のパーセンテージとして定義します。 パーセンテージは、幅の絶対数から計算します。
...
* media-welcome ページのデフォルト CSS により、 `float:left` スタイル `span*` 内部にあるクラス `row-fluid` クラス。
...
#### ページコンポーネントをモジュール化する {#tip-modularize-your-page-components}
...
ページの様々な部分（例： ）を生成するためのスクリプトを提供するメインページコンポーネントを作成します。 `head` および `body` セクションと `header`, `content`、および `footer` セクションを追加します。
...
複数のページコンポーネントに共通する行と列の組み合わせを生成する、複数の JSP スクリプトを作成します。例えば、 `content.jsp` 記事のスクリプトと media-home コンポーネントの両方が `8x4col.jsp` スクリプト
...
この `.row-fluid .span6` ページの CSS のセレクターが `div` このHTML内の同じクラスと構造体の要素。 ただし、このコンポーネントには、 /apps/geometrixx-media/components/2-col-article-summary/clientlibs クライアントライブラリフォルダーも含まれます。
...
例えば、次のスタイルが `/apps/geometrixx-media/components/2-col-article-summary/clientlibs/css/responsive-480px.css` ファイル：
...
* HTML `div` 要素には、特定の数の列にまたがるコンテンツのブロックが含まれます。
...
* のクラス `div` すべての行のコンテナである要素。 このクラスは、グリッドの幅の絶対値を設定します。
* のクラス `div` 行を表す要素。 このクラスでは、格納するコンテンツブロックの水平方向または垂直方向の位置を制御します。
...
1. 行を表す div 要素の CSS クラスを定義する（例： ） `row-fluid`.
1. コンテンツブロック要素のクラス名を定義します。クラスは、列の範囲に関して、可能なすべての幅に必要です。 例えば、 `span3` クラス `div` 3 列にまたがる要素、を使用 `span4` 4 列の範囲のクラス。 グリッドに列がある数だけクラスを定義します。
...
1. 各セレクターに幅のスタイルを追加します。
   ...
   1. 幅を `.row-fluid .span` セレクターは、合計の幅に対する割合で指定します。 この幅を計算するには、 `(absolute span width)/(absolute page width)*100` 数式

#### 行内のコンテンツブロックの配置 {#positioning-content-blocks-in-rows}

フロートスタイルを使用する `.row-fluid` クラスを使用して、行のコンテンツブロックを水平方向または垂直方向に配置するかどうかを制御します。

* この `float:left` または `float:right` style は、子要素（コンテンツブロック）を水平方向に分布します。

* この `float:none` style は、子要素を垂直方向に分布させます。

次にスタイルを追加： `.row-fluid` 各メディアクエリ内のセレクター そのメディアクエリに使用するページレイアウトに従って値を設定します。 例えば、次の図は、幅の広いビューポートではコンテンツを水平方向に、幅の狭いビューポートでは垂直方向に配分する行を示しています。
