...
Tough Day 2 は、AEMインスタンスの制限を強調テストできるアプリケーションです。 Tough Day 2 は、デフォルトのテストスイートを使用してそのまま実行することも、テストのニーズに合わせて設定することも可能です。このアプリケーションのプレゼンテーションについては、[こちらの録画](https://docs.adobe.com/ddc/en/gems/Toughday2---A-new-and-improved-stress-testing-and-benchmarking-tool.html)を参照してください。
...
スイートのテストを実行するために、Tough Day 2 によってデフォルトのコンテンツパッケージがインストールされます。これは、`installsamplecontent` パラメーターを `false` に設定することで回避できますが、実行するテストのデフォルトパスも変更する必要があります。 jar がパラメーターなしで実行されている場合、Tough Day 2 には [ ヘルプ情報 ](/help/sites-developing/tough-day.md#getting-help) が表示されます。
...
Tough Day 2 には、コマンドラインからアクセスできる幅広いヘルプオプションが用意されています。例えば、次の操作が可能です。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>--help</td> 
   <td>グローバル情報を出力します。次に例を示します。使用可能なアクション、定義済みのスイート、実行モードおよびグローバルパラメーター</td> 
   <td> </td> 
  </tr> 
  <tr> 
   <td>—help_publish</td> 
   <td>使用可能なすべてのパブリッシャーを印刷します。</td> 
   <td> </td> 
  </tr> 
  <tr> 
   <td>—help_tests</td> 
  ...
  <tr> 
   ...
   <td><p>java -jar toughday2.jar —help PDFTest のアップロード</p> <p>java -jar toughday2.jar —help CSVPublisher</p> </td> 
  </tr> 
 </tbody> 
</table>
...
Tough Day 2 には、テストの環境を設定または変更するグローバルパラメーターが用意されています。これには、ターゲットのホスト、ポート番号、使用されるプロトコル、インスタンスのユーザーとパスワードなどが含まれます。例えば、次の操作が可能です。
...
| `--suite=<Val>` | 値は、事前定義済みのテストスイートの 1 つまたは（コンマ区切りの）リストにすることができます。 | 厳しい |  |
...
| `--dryrun=<Val>` | true の場合、結果の設定が出力され、テストは実行されません。 | false | true または false |
...
テスト設定を保存するには、yaml 形式でコピーする方法しかありません。詳しくは、この [toughday.yaml](https://repo.adobe.com/nexus/service/local/repositories/releases/content/com/adobe/qe/toughday2/0.2.1/toughday2-0.2.1.yaml) の設定と以下の節の yaml 設定の例を参照してください。
...
同じテストの複数のインスタンスを追加して実行することもできますが、そのためにはそれぞれのインスタンスに一意の名前を付ける必要があります。以下の例では、コマンドラインパラメーターまたは yaml 設定ファイルを使用して、同じテストの 2 つのインスタンスを追加する方法を示します。
...
1 つ以上のテストプロパティを変更する必要がある場合は、そのプロパティをコマンドラインまたは yaml 設定ファイルに追加できます。使用可能なすべてのテストプロパティを表示するには、次のように `--help <TestClass/PublisherClass>` パラメーターをコマンドラインに追加します。
...
次の例は、コマンドラインパラメーターまたは yaml 設定ファイルを使用して `CreatePageTreeTest` テストの `template` プロパティを変更する方法を示しています。
...
特定のスイート内の既存のテストは、 `config`* *パラメーターを使用して再設定することもできます。 スイート名とテストの実際の名前（テストクラス名ではない）も指定する必要があります。 テスト名は、テストクラスの `name` プロパティで確認できます。テストプロパティの確認方法について詳しくは、[テストプロパティの変更](/help/sites-developing/tough-day.md#changing-the-test-properties)を参照してください。
...
さらに、`exclude` パラメーターを使用して、事前定義済みのスイートからテストを削除したり、デフォルトの設定から公開者を削除したりできます。また、（テスト C の `lass` 名ではなく）スイート名とテストの実際の名前も指定する必要があります。 テスト名は、テストクラスの `name` プロパティにあります。 次の例では、 `CreatePageTreeTest` （`UploadAsset` という名前）テストが toughday スイートから削除されています。
...
Tough Day 2 は、次のいずれかのモードで実行できます。**通常の** および **定負荷**。

**通常の** 実行モードには、次の 2 つのパラメーターがあります。
...
**定負荷** 実行モードは、一定数のスレッドではなく、一定数の開始テスト実行を生成することで、通常の実行モードとは異なります。 同じ名前の実行モードパラメーターを使用して、読み込みを設定できます。

### テストの選択 {#test-selection}

テストの選択プロセスは、両方の実行モードで同じで、次のように実行されます。すべてのテストには `weight` プロパティがあり、スレッドでの実行の可能性を判断します。 例えば、テストが 2 つあり、一方の重みを 5、もう一方の重みを 10 とした場合、後者が実行される可能性は前者の 2 倍になります。

さらに、テストには `count` プロパティを設定できます。このプロパティを使用すると、実行回数を指定した数に制限できます。 この回数に達すると、それ以上テストは実行されません。既に実行中のテストインスタンスはすべて、設定されたとおりに実行を終了します。次の例は、コマンドラインまたは yaml 設定ファイルを使用してこれらのパラメーターを追加する方法を示しています。
...
>[!NOTE]
>
>並列実行により、実際のテスト実行回数が `count` パラメーターで設定した数と正確に一致するわけではありません。 偏差は、（`concurrency parameter` によって制御される）実行中のスレッドの数に比例します。
...
現在、Tough Day 2 では、ユーザーが評価できる 9 種類のテスト指標が報告されます。**&amp;ast;** 記号を含む指標は、正常に実行された後にのみ報告されます。
...
| 渡された | 成功した実行の数。 |
| 失敗 | 失敗した実行の数。 |
| 最小アンプ；ast; | テスト実行の最も低い期間。 |
| Max&amp;ast; | テスト実行の最高時間。 |
| 中央値 (&amp;A);ast; | すべてのテスト実行の計算済み中央値時間。 |
| Average&amp;ast; | すべてのテスト実行の計算された平均時間。 |
| StdDev&amp;ast; | 標準偏差。 |
| 90p&amp;ast; | 90 百分位 |
| 99p&amp;ast; | 99 百分位 |
| 99.9p&amp;ast; | 99.9 百分位 |
| 実際のスループット (&amp;ast;) | 実行回数を経過した実行時間で割った値。 |
...
* **シンプル** 公開モード — 実行の開始から公開時点までの結果を報告します。
* **intervals** パブリッシュモード — 所定の期間で結果を報告します。 この時間枠は、 **interval** パブリッシュモードパラメーターを使用して設定できます。
...
Tough Day 2 では、Tough Day 2 を実行したディレクトリと同じディレクトリにログフォルダーが作成されます。 このフォルダーには次の 2 種類のログが格納されます。
