...
システムが提供するスクリプトが存在します ( `/libs/sling/servlet/errorhandler`) を使用してエラーコードに応答します。デフォルトでは、標準の CQ インスタンスでは次の処理を使用できます。
...
>[!NOTE]
>
>AEMは Apache Sling に基づいているので、 [https://sling.apache.org/site/errorhandling.html](https://sling.apache.org/site/errorhandling.html) を参照してください。
...
独自のスクリプトを作成して、エラーの発生時にエラーハンドラーで表示されるページをカスタマイズできます。カスタマイズしたページがの下に作成されます。 `/apps` デフォルトのページ ( `/libs`) をクリックします。
...
要求の処理結果が例外になると、(AEMが構築されている )Apache Sling フレームワークは次のようになります。
