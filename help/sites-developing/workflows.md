...
AEMには、ワークフローモデルの作成、ワークフローステップの開発、およびプログラムによるワークフローとの対話を行うためのツールとリソースがいくつか用意されています。
...
>参考情報：
>
>* ワークフローへの参加 ( [ワークフローの使用](/help/sites-authoring/workflows.md).
>* ワークフローとワークフローインスタンスの管理については、[ワークフローの管理](/help/sites-administering/workflows.md)を参照してください。
>* コミュニティに関するエンドツーエンドの記事については、 [Adobe Experience Manager Workflows を使用したデジタルアセットの変更](https://helpx.adobe.com/experience-manager/using/modify_asset_workflow.html)
>* 詳しくは、 [ワークフローに関するAEM Experts のウェビナーへの質問](https://bit.ly/ATACE218).
>* コミュニティに関するエンドツーエンドの記事については、 [カスタムAdobe Experience Manager 6.3 動的参加者ステップの作成](https://helpx.adobe.com/experience-manager/using/dynamic-steps-aem63.html).
...
`WorkflowModel` は、ワークフローの定義（モデル）を表します。これは、 `WorkflowNodes` および `WorkflowTransitions`. トランジションはノードを接続し、 *流れ*. モデルには常に開始ノードと終了ノードがあります。
...
発生するワークフローモデル、および生成されるランタイムモデルの編集 *後* 特定のインスタンスが開始された場合、そのインスタンスには適用されません。
...
A `WorkflowTransition` 2 つの間の遷移を表す `WorkflowNodes` の `WorkflowModel`.
...
A `WorkItem` は、 `Workflow` のインスタンス `WorkflowModel`. これには、 `WorkflowData` インスタンスが機能し、 `WorkflowNode` 基になるワークフローの手順を示します。

* タスクを識別するために使用され、それぞれのインボックスに入れられます。
* ワークフローインスタンスには、1 つ以上の `WorkItems` 同時に（ワークフローモデルに応じて）実行します。
...
各ユーザーアカウントには、割り当てられた独自のワークフローインボックスがあります `WorkItems` はアクセス可能です。
...
* カスタムワークフロー（コンソールにインジケーターが表示されません）

   これらは、新規に作成されたワークフロー、または標準のワークフローから作成されたワークフローで、カスタマイズが適用された状態でオーバーレイされています。
...
>
>これは、 **ステップに移動** は sling ジョブを作成し、 `goto` ポイント これにより、ワークフローを一時的にする目的が損なわれ、ログファイルにエラーが生成されます。
>
...
有効化中 **マルチリソースのサポート** ワークフローモデルの場合、複数のリソースを選択した場合でも、単一のワークフローインスタンスが開始されます。これらはパッケージとして添付されます。
...
1. ワークフローが実行されると、ユーザーは進行状況を（ステップ名ではなく）ステージ名に照らして確認できます。ワークフローの進行状況が [作業項目のタスクの詳細ウィンドウの「ワークフロー情報」タブ](/help/sites-authoring/workflows-participating.md#opening-a-workflow-item-to-view-details-and-take-actions) 次に示す [インボックス](/help/sites-authoring/inbox.md).

### ワークフローおよびフォーム {#workflows-and-forms}

一般的には、ワークフローは AEM でのフォーム送信の処理に使用されます。これは、 [コアコンポーネントのフォームコンポーネント](https://helpx.adobe.com/experience-manager/core-components/using/form-container.html) 標準のAEMインスタンスで、または [AEM Formsソリューション](/help/forms/using/aem-forms-workflow.md).
