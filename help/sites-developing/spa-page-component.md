...
>[!CAUTION]
>
>このドキュメントでは、 We.Retail ジャーナルアプリをデモ目的でのみ使用します。 どのプロジェクト作業にも使用しないでください。
>
>任意のAEMプロジェクトで [AEM Project Archetype](https://docs.adobe.com/content/help/ja-JP/experience-manager-core-components/using/developing/archetype/overview.html)は、React またはAngularを使用したSPAプロジェクトをサポートし、SPA SDK を活用します。AEMのすべてのSPAプロジェクトは、SPA Starter Kit 用の Maven アーキタイプに基づく必要があります。
...
これは、SPAサンプルコンテンツ ( `/conf/we-retail-journal/react/settings/wcm/policies/we-retail-journal/react/components/structure/page/root`.