...
JCR DATE プロパティと日時の間隔を照合します。これはISO8601を使用します。\
日付と時間の形式( `YYYY-MM-DDTHH:mm:ss.SSSZ`)を使用し、同様に部分的な表現も使用できま `YYYY-MM-DD`す。 また、ミリ秒数のタイムスタンプ（UTC タイムゾーン、UNIX 時刻形式、1970 年以降）を指定することもできます。
