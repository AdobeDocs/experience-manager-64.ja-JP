...
>
>現在では、HTL が AEM の推奨スクリプティング言語とされています。手法を比較するには、[HTL](https://helpx.adobe.com/jp/experience-manager/htl/user-guide.html) および [AEM コンポーネントの開発](/help/sites-developing/developing-components.md)を参照してください。
...
* `global.jsp` に設定されている `currentPage` オブジェクト経由：
   ...
   `currentPage` オブジェクトは、ページのインスタンスです（[AEM API](https://helpx.adobe.com/jp/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/Page.mhtml) を参照）。ページクラスには、コンテンツにアクセスするためのメソッドがいくつかあります。
...
1. 新しいコンポーネントフォルダーを開き、必要な変更を行います。 また、フォルダー内の不要な情報を削除します。
   ...
   * または、コンポーネント全体の作成し直し（必要な場合）
   ...
   >* クラシック UI の使用 [ExtJS ウィジェット](https://helpx.adobe.com/jp/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html)
...
1. サイドキックの「デザイン」をクリックして、デザインモードに切り替えます。
