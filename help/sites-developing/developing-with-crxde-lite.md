...
CRXDE Lite は AEM に搭載されており、これを使用してブラウザー内で標準的な開発作業を実行できます。CRXDE Liteを使用すると、ログ記録時に、プロジェクトの作成、ファイル（.jspや.javaなど）の作成および編集、フォルダー、テンプレート、コンポーネント、ダイアログ、ノード、プロパティおよびバンドルを実行できます。
