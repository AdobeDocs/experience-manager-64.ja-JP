...
上記の実装では、プロジェクトコンポーネントは、実際にコンポーネントマッピングストアに登録される前に、空白機能で拡張 [されます](/help/sites-developing/spa-blueprint.md#componentmapping) 。 これは、設定オブジェクトのサポートを導入する [ために](#componentmapping) 、ComponentMappingライブラリをカプセル化し拡張することで `EditConfig` 行われます。
...
基になるラ [`PageModelManager`](/help/sites-developing/spa-blueprint.md#pagemodelmanager) イブラリとその `[`ModelRouter`](/help/sites-developing/spa-routing.md)モジュール（デフォルトで有効）は、特定のリソースパスに関連付けられたモデルに対するプリフェッチとアクセスを行います。

2つのエンティティはルーティングの概念に関連していますが、 [`ModelRouter`](/help/sites-developing/spa-routing.md) は、現在のアプリケーションの状態と同期する構造のデータモデルで [`PageModelManager](/help/sites-developing/spa-blueprint.md#pagemodelmanager) を読み込むだけを行います。
